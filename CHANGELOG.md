...
- **A with-sparsity headline halved is not a datasheet row, and now says so.** Each entry records `tflops_basis` (`dense` / `halved-with-sparsity` / `derived-2x-fp32` / `fp32-accumulate-dense`). The halving heuristic is not universally safe -- AMD publishes both columns for some parts and no sparsity figure at all for others, where halving the headline would understate a dense number by 2x.

### Added
- **An unlisted GPU is no longer a wall.** `plan --hardware "RTX 6090 48GB"` exited 1 with a list of known cards. The planner is *model*-agnostic by design and was *hardware*-locked by accident: an unreleased model can be planned with `--params-b`, and an unlisted GPU could not be planned at all. `--gpu-vram-gb` and `--gpu-bandwidth-gbps` (plus optional `--gpu-fp16-tflops`, `--gpu-tdp-w`, `--gpu-interconnect-gbps`, `--gpu-price-per-hour`) now plan any card. A supplied spec is marked as the user's, carries no source URL, and **refuses FP8** rather than assuming it -- support is not knowable from VRAM and bandwidth, and offering it on a part that emulates it hands out a config that does not run. A field nobody supplied stays unknown rather than inheriting the reference card's value, which closes the latent silent default behind that wall (`bandwidth_ratio()` returned 1.0 for an unknown GPU and the roofline fell back to 556 GB/s).
- **`plan --hardware auto`** reads the installed card via pynvml or `nvidia-smi`. A driver name is matched on its model, longest-first, and disambiguated by the VRAM the driver reported -- `get_gpu`'s substring match could not connect `"NVIDIA GeForce RTX 4090"` to the key `"RTX 4090 24GB"`, so `auto` would have failed on the most common consumer card in the database. An **unmatched** device reports the driver's name and VRAM and refuses rather than inferring bandwidth from the name: a wrong name match produces a fully provenance-labelled plan for a card the user does not have. An ambiguous model with no VRAM figure is not matched at all, since guessing 8 GB for a 16 GB card silently halves every VRAM verdict.
...
- **The sort ordered configurations on sub-noise quality gaps.** Two cells whose intervals overlap now tie and fall through to the next key instead of being ranked on a 0.4pp difference that 20 items cannot see.
- **Quality was context-independent, and cannot be.** The same 4-bit configuration is near-lossless at 2K and loses up to 59% on long-context inputs (arXiv:2505.20276, 9.7K test examples across five quantization methods and five models). A cell measured at 2K no longer licenses a verdict at 64K: narrow quants report UNKNOWN there, naming the source, instead of extrapolating. 8-bit is reported as near-lossless and keeps its cell, which is what the paper says.
...
- **The SSM state dtype is read, not assumed.** `Qwen3.5-9B` declares `mamba_ssm_dtype: float32` while the model itself is bfloat16; taking the model dtype would have halved the term that dominates.
...
- **`chimeraforge mcp` printed an install command that fixes nothing.** The error text is `pip install "chimeraforge[mcp]"`, but it was interpolated into a Rich renderable unescaped, so `[mcp]` was parsed as a style tag and deleted -- the one message whose entire job is to hand the user the fix handed them `pip install "chimeraforge"`. Escaped here and at four sibling sites in `eval`, `measure` and `safety`.
- **`python -m chimeraforge` exited 0 having printed nothing.** There was no `__main__.py` and no `__name__` guard, so the conventional module invocation silently did nothing at all -- the worst possible response, since it looks like the command ran.
- **The sdist shipped a test suite that could not run.** `tests/*.py` were included but `conftest.py`, `helpers.py` and `fixtures/` were not, so `pytest tests/` from an unpacked sdist failed collection outright. Downstream packagers build from the sdist and run the suite as their gate. A `MANIFEST.in` now ships what the suite needs: **1470 pass from a clean sdist**, against four collection errors before.
- **`workflow_dispatch` bypassed the release version guard.** The tag-matches-pyproject check sat behind `if: github.event_name == 'release'`, so a manual dispatch from any branch skipped the only version check and would publish whatever that branch declared. It now runs on both triggers.

### Documentation
- **Three README performance figures were stale, unreproducible, or silently hardware-specific**, against the project's own rule that provenance travels with a value. The reasoning-token claim (363ms -> 6128ms) reproduced on **no GPU in the database** and is now 193ms -> 3664ms on an H100; the at-capacity cost of $0.92 predated the 0.30.4 recalibration and is $0.71; the prefix-cache figure was correct but only on an H100, which the sentence never named. Each now carries the exact command that produces it.
- **The `--fleet` example no longer produced a mix.** 0.30.4's recalibration raised every non-reference GPU ~29%, so one H100 now serves 82.6 req/s and the README's 64 req/s example never needs a second GPU -- the feature's only worked justification was a result the tool could not produce. Re-derived at 250 req/s, where 3x H100 + 1x L4 genuinely beats 6x A100 by 16.7%.
- **`--expect-fingerprint` is documented.** The README described the pre-registration guarantee unconditionally while the flag that actually delivers it (added in 0.30.9) appeared nowhere.
- The `[eval]` extra promised BERTScore but installs only `evaluate`, the metric loader; `bert-score` and torch are in no extra. The gated `meta-llama` example is now an ungated Qwen repo that runs without a token, the RTX 30-series is no longer described as Ada, and the `pip install` extras are quoted so they work in zsh.

### Testing
- **An independent mutation audit ran 30 mutations; 12 survived a fully green suite.** Every survivor that matters is now pinned, each verified by re-running the mutation: the percentile interpolation used by every bench p50/p95/p99 (swapping its weights moved a p95 from 173.0 to 117.0 with nothing failing), `SECONDS_PER_MONTH` (halving the billing month passed, because the test derived its expectation from the same constant), `QUANT_BPW["Q4_K_M"]` (an 11% VRAM error on the default recommendation), the 70% utilisation safety cap (no test referenced it or its warning at all), the Amdahl serial fractions that drive the replica search, `DECODE_COMPUTE_MFU`, `INTERCONNECT_EFFICIENCY` and the agent variance preset.
- **The ollama entry added to the launch-quoting test was itself vacuous.** `build_launch_command` was called with `spec=None`, so the tag resolved to a placeholder and the hostile string never reached the command -- the assertion short-circuited on `"$(id)" not in cmd`. It now supplies an Ollama-sourced spec and asserts the tag arrives quoted.
...
- **MAPE returned 0.0 when every pair was filtered out** for a zero actual -- flawless accuracy computed from no usable observations. Now `None`.
- **Empty model replies diluted the refusal rate toward the unsafe reading.** `classify_refusal("")` is False, so 20 empty responses gave `refusal_rate 0.000 (measured)` from a run that produced no information. They are now excluded from the denominator as errors already were, counted in `n_empty`, and disclosed.
- **`eval --json` attributed scores to a model it never queried.** Without `--predictions` the task prompts are scored as the answers, and the disclaimer printed only when `--json` was absent -- so exactly the machine-readable path lost it. The payload now carries `predictions_source: "placeholder"` and a warning.
- **A missing `bert-score` silently reweighted the composite**, so identical predictions scored 0.580/negligible with it installed and 0.460/concerning without, under identically-named fields. The score now carries `composite_weights`.

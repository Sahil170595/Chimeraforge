...
def analyze_runs(models_data: dict):
    ...
    for model_name, data in models_data.items():
        ...
        print(
            f"    Std: {statistics.stdev(latencies) if len(latencies) > 1 else 0:.2f} ms"
        )
        ...
        print("  Warmup latencies:")
        if warmup_latencies:
            print(f"    Mean: {statistics.mean(warmup_latencies):.2f} ms")
            print(
                f"    First warmup: {warmup_latencies[0] if warmup_latencies else 'N/A':.2f} ms"
            )
        print()
...
def analyze_all_scenarios():
    ...
    for model in models:
        csv_path = Path(
            f"research/tr118/results/tr118v2_crossover/{model}/processed/latency_summary_prefill.csv"
        )
        ...
        with open(csv_path, encoding="utf-8") as f:
            ...
            for scenario in scenarios:
                pytorch = [
                    r
                    for r in rows
                    if r["backend"] == "transformers-gpu-compile"
...
def main():
    ...
    models = {
        "50m": {
            "raw": Path("research/tr118/results/tr118v2_crossover/gpt2-50m/raw"),
            "model": Path("models/gpt2-50m"),
        },
        "75m": {
            "raw": Path("research/tr118/results/tr118v2_crossover/gpt2-75m/raw"),
            "model": Path("models/gpt2-75m"),
        },
        "100m": {
            "raw": Path("research/tr118/results/tr118v2_crossover/gpt2-100m/raw"),
    ...
    for name, paths in models.items():
        ...
        filtered_runs = [
            r
            for r in runs
            if r.get("spec", {}).get("scenario") == "single_medium"
            and r.get("spec", {}).get("backend") == "transformers-gpu-compile"
        ]

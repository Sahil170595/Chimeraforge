        ...
        **kwargs,
        ...
        if padding_mask is not None:
            with contextlib.suppress(Exception):
                padding_mask = mu.prepare_padding_mask(
                    padding_mask, kv_length, kv_offset
                )
        ...
        try:
            if (
                ...
                and hasattr(mu, "_ignore_causal_mask_sdpa")
                and mu._ignore_causal_mask_sdpa(  # type: ignore[attr-defined]
                    padding_mask, seq, seq, kv_offset, local_size
                )
            ):
                return None
        except Exception:
        ...
        if sliding_window is not None and chunk_size is not None:
            raise ValueError(
                "Cannot use both `sliding_window` and `attention_chunk_size`"
            )
        if sliding_window is not None:
            sliding_mask_overlay = kv_arange > (
                reshaped_cache_position - int(sliding_window)
            )
            causal_mask = causal_mask & sliding_mask_overlay
        elif chunk_size is not None:
            chunk = int(chunk_size)
            chunked_mask_overlay = (kv_arange // chunk) == (
                reshaped_cache_position // chunk
            )
        ...
        try:
            if (
                not getattr(mu, "_is_torch_greater_or_equal_than_2_5", True)
                and allow_torch_fix
            ):
                causal_mask = causal_mask | torch.all(
                    ~causal_mask, dim=-1, keepdim=True
                )
        except Exception:
            pass
...
def export_model_to_onnx(
    ...
    try:
        ...
        if trt_friendly_inputs:
            sample_inputs = {
                k: v.to(dtype=torch.int32) for k, v in sample_inputs.items()
            }
        ...
        logger.info(
            f"Exporting to ONNX (opset={opset_version}, dynamic={dynamic_axes})..."
        )
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        f"ONNX export failed after {max_retries} attempts: {exc}"
                    )
                    raise
        ...
        try:
            if model is not None:
                metadata["model_weight_files"] = inspect_model_weight_files(
                    model_name_or_path
                )
        except Exception:
            pass
    ...
    logger.info(
        f"Export complete: {output_path} ({file_size_mb:.2f} MB) in {export_time:.2f}s"
    )
    ...
    return {
        ...
        "onnx_inspect": (
            inspect_onnx_artifact(output_path) if output_path.exists() else None
        ),
...
def main():
    ...
    parser.add_argument(
        "--static-shapes",
        action="store_true",
        help="Use static shapes (no dynamic axes)",
    )
    parser.add_argument(
        "--trt-friendly",
        action="store_true",
    ...
    parser.add_argument(
        "--max-retries", type=int, default=3, help="Maximum retry attempts for export"
    )
    parser.add_argument(
        ...
        if args.metadata:
            try:
                ...
                error_metadata = {
                    "model": args.model,
                    "output_path": (
                        str(out_path) if "out_path" in locals() else str(args.output)
                    ),
                    "success": False,
                    "error": str(exc),
                    "error_type": type(exc).__name__,

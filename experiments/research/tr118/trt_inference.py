...
class TensorRTInferenceEngine:
    def __init__(self, engine_path: Path):
        ...
        if self._api == "bindings":
            ...
            bindings_per_profile = (
                int(engine.num_bindings // self._num_profiles)
                if self._num_profiles > 0
                else engine.num_bindings
            )
            if (
                self._num_profiles > 0
                and bindings_per_profile * self._num_profiles != engine.num_bindings
            ):
    ...
    def _binding_indices_base(self) -> tuple[int, int, int]:
        ...
        idx_out = next(
            (i for i in range(engine.num_bindings) if not engine.binding_is_input(i)),
            -1,
        )
    ...
    def _profile_shapes(
        self, profile_idx: int, binding_name: str
    ...
    def select_profile(
        self, batch: int, seq: int, input_binding_name: str = "input_ids"
    ) -> int:
    ...
    def prepare(self, batch: int, seq: int, vocab_size: int) -> dict[str, Any]:
        ...
        if self._api == "tensors":
            ...
            if not bool(ctx.set_input_shape(input_ids_name, (batch, seq))):
                raise RuntimeError(
                    f"set_input_shape_failed:{input_ids_name}:{batch}x{seq}"
                )
            if not bool(ctx.set_input_shape(attention_mask_name, (batch, seq))):
                raise RuntimeError(
                    f"set_input_shape_failed:{attention_mask_name}:{batch}x{seq}"
                )
    ...
    def infer(
        self, prepared: dict[str, Any], input_ids, attention_mask
    ) -> tuple[float, Any]:
        cuda = self._cuda
        ctx = self._context
        stream = self._stream
        ...
        if self._api == "tensors":
            ...
            ctx.execute_async_v2(
                bindings=prepared["bindings"], stream_handle=stream.handle
            )

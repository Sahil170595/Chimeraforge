...
Outputs:
- `research/tr118/results/processed/perplexity_results.csv`
- `research/tr118/results/processed/perplexity_results.json`
"""
...
def _load_wikitext_texts(dataset_config: str, split: str, limit: int) -> list[str]:
    try:
        ...
        raise RuntimeError(
            "datasets package required for perplexity evaluation"
        ) from exc
    ...
    for i in range(0, len(texts), batch_size):
        ...
        if input_ids.shape[0] < batch_size:
            ...
            input_ids = np.concatenate(
                [input_ids, np.zeros((pad_rows, max_length), dtype=np.int32)], axis=0
            )
            attention_mask = np.concatenate(
                [attention_mask, np.zeros((pad_rows, max_length), dtype=np.int32)],
                axis=0,
            )
...
def _reference_check_pytorch_loss(
    model, tokenizer, texts: list[str], max_length: int, device: str
) -> dict[str, Any]:
    import torch  # type: ignore
...
def main() -> int:
    ...
    results_dir = (
        resolve_repo_path(_REPO_ROOT, str(cfg["output"]["results_dir"])) / "processed"
    )
    ...
    baseline_backend = (
        str(cfg.get("baseline", {}).get("backend", "pytorch")).strip() or "pytorch"
    )
    ...
    try:
        ...
        if device != "cpu":
            logger.warning(
                "Failed to load PyTorch model on %s (%s); falling back to CPU",
                device,
                exc,
            )
            device = "cpu"
    ...
    if onnx_path.exists():
        ...
        for backend, providers in [
            ...
            try:
                ...
                delta = (
                    (stats.perplexity - base.perplexity) / base.perplexity
                    if base.perplexity
                    else 0.0
                )
                results.append(
    ...
    out_json.write_text(
        json.dumps({"results": results, "diagnostics": diagnostics}, indent=2),
        encoding="utf-8",
    )

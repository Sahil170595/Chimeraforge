...
def _replace_section(
    text: str, start_heading: str, end_heading: str, new_section: str
) -> str:
        ...
        for path in root.glob("bench_*_*.jsonl"):
            ...
            for line in path.read_text(encoding="utf-8").splitlines():
                ...
                stats.setdefault(
                    backend,
                    {
                        "prefill": {"total": 0, "degraded": 0},
                        "generate": {"total": 0, "degraded": 0},
                    },
                )
...
def main() -> int:
    ...
    audit_dir = repo_root / "research/tr118/results/tr118v2_audit"
    ...
    tiny_prefill = (
        repo_root
        / "research/tr118/results/tr118v2/20251213_135135_deep/tiny-gpt2/processed/latency_summary_prefill.csv"
    )
    gpt2_prefill = (
        repo_root
        / "research/tr118/results/tr118v2/20251213_135135_deep/gpt2/processed/latency_summary_prefill.csv"
    )
    ...
    text = _replace_section(
        text, "## Executive Summary", "## Table of Contents", exec_summary
    )
    ...
    for point in points:
        ...
        rows.append(
            "| {model} | {params} | {onnx} | {pt} | {ratio} |".format(
                model=(
                    Path(point["model_ref"]).name if point["model_ref"] else "unknown"
                ),
    ...
    text = _replace_section(
        text, "### 10.3 Future Work", "## 11. Reproducibility", section_10_3
    )
    ...
    stats = _backend_success_table(
        [
            repo_root
            / "research/tr118/results/tr118v2/20251213_135135_deep/tiny-gpt2/raw",
            repo_root / "research/tr118/results/tr118v2/20251213_135135_deep/gpt2/raw",
        ]
    )
    rows = []
    ...
    text = text.replace(
        "All degraded (100% timeout)", "All degraded (profile mismatch)"
    )
    text = text.replace(
    ...
    text = text.replace(
        "### 16.1 Definitive Findings", "### 16.1 Quick Reference Findings"
    )
    text = text.replace(
        "### 16.2 Updated Decision Matrix", "### 16.2 Decision Matrix (Prefill-Only)"
    )

...
Output:
- Run-level JSONL under `research/tr118/results/raw/bench_<mode>_<ts>.jsonl`
...
from research.shared.artifact_utils import (
    file_sha256,
...
from research.tr118.build_trt_engines import build_tensorrt_engine
from research.tr118.export_to_onnx import export_model_to_onnx
from research.tr118.onnx_sanitize import sanitize_onnx_file
from research.tr118.trt_inference import TensorRTInferenceEngine

try:
    from research.shared.instrumentation import ResourceMonitor  # type: ignore
...
def _load_prompt_config(
    ...
    cfg = _load_yaml(path)
    prompt_sets = {
        k: [str(p) for p in v] for k, v in cfg.get("prompt_sets", {}).items()
    }
    scenarios = {}
...
def _maybe_export_onnx(
    cfg: dict[str, Any], meta_path: Path
    ...
    force_export = (
        bool(cfg.get("onnx", {}).get("force_export", False))
        or os.getenv("BANTER_TR118_FORCE_EXPORT", "0") == "1"
    )
        ...
        try:
            ...
            if isinstance(existing, dict):
                ...
                if "model_weight_files" not in existing:
                    existing["model_weight_files"] = inspect_model_weight_files(
                        model_name
                    )
    ...
    if onnx_path.exists() and not force_export:
        ...
        meta = {
            ...
            "export_time_s": None,
            "file_size_mb": (
                float(onnx_path.stat().st_size / (1024 * 1024))
                if onnx_path.exists()
                else None
            ),
    ...
    if skip_export and not force_export:
        if onnx_path.exists():
            meta = {
                ...
                "opset_version": int(cfg["onnx"]["opset_version"]),
                "dynamic_axes": bool(cfg["onnx"].get("dynamic_axes", True)),
                "trt_friendly_inputs": bool(
                    cfg["onnx"].get("trt_friendly_inputs", True)
                ),
                "valid": None,
                "reused": True,
                "skipped": True,
    ...
    try:
        ...
        if not dynamic_axes:
            try:
                ...
                static_seq_len = int(
                    cfg.get("onnx", {}).get(
                        "static_seq_len",
                        cfg.get("benchmark", {}).get("max_seq_len", 512),
                    )
                    or 512
                )
                ...
                if static_batch > 1:
                    ...
                    enc["attention_mask"] = enc["attention_mask"].repeat(
                        static_batch, 1
                    )
                sample_inputs = {
                ...
                logger.warning(
                    "Failed to build static sample_inputs; falling back to exporter defaults: %s",
                    exc,
                )
                sample_inputs = None
        ...
        try:
            ...
            pass
        logger.warning(
            "ONNX export failed; continuing with existing artifacts: %s", exc
        )
        return onnx_path, None
...
def _maybe_build_trt(
    ...
    if isinstance(int8_cal, dict) and int8_cal.get("cache_path"):
        ...
        int8_cal["cache_path"] = str(
            resolve_repo_path(_REPO_ROOT, str(int8_cal["cache_path"]))
        )
    ...
    force_rebuild = (
        bool(cfg.get("tensorrt", {}).get("force_rebuild", False))
        or os.getenv("BANTER_TR118_FORCE_TRT_REBUILD", "0") == "1"
    )
        ...
        try:
            ...
            if isinstance(existing, list) and existing:
                have = {
                    str(r.get("precision"))
                    for r in existing
                    if isinstance(r, dict) and r.get("precision")
                }
                if all(p in have for p in precisions) and all(
                    engines[p].exists() for p in precisions
                ):
                    changed = False
                    for r in existing:
    ...
    for precision in precisions:
        ...
        base_meta = {
            ...
            "builder_settings": (
                builder_settings if isinstance(builder_settings, dict) else {}
            ),
            "int8_calibration_config": (
                int8_cal
                if (precision == "int8" and isinstance(int8_cal, dict))
                else None
            ),
        ...
        try:
            meta = build_tensorrt_engine(
                ...
                builder_settings=(
                    builder_settings if isinstance(builder_settings, dict) else None
                ),
            )
            meta["built"] = True
            meta["reused"] = False
            ...
            if (
                backup_plan is not None
                and backup_plan.exists()
...
def _compute_seq_len(
    tokenizer, prompts: list[str], max_len: int, min_len: int = 1
) -> int:
...
def _infer_ort(
    session, input_type: str, tokenizer, prompt: str, seq_len: int, batch_size: int
) -> tuple[float, str]:
    dtype = "int32" if "int32" in input_type else "int64"
    input_ids, attention_mask = _to_numpy_inputs(
        tokenizer, prompt, seq_len, batch_size, dtype=dtype
    )
...
def _infer_ort_logits(
    session, input_type: str, input_ids, attention_mask
) -> tuple[float, Any]:
    import numpy as np  # type: ignore
...
def _infer_torch_logits(
    model, input_ids, attention_mask, device: str
) -> tuple[float, Any, bool, str | None]:
    try:
        import torch  # type: ignore
    except Exception as exc:
    ...
    try:
        ...
        with torch.inference_mode():
            start = time.perf_counter()
            out = model(
                input_ids=input_ids_t, attention_mask=attention_mask_t, use_cache=False
            )
            logits = out.logits
            logits_np = logits.detach().cpu().numpy()
            if device.startswith("cuda") and torch.cuda.is_available():
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="research/tr118/configs/baseline.yaml",
    ...
    parser.add_argument(
        ...
        default=os.getenv(
            ...
            "research/tr117/configs/matrix_tier3.yaml",
        ),
    ...
    run_id = (
        str(args.run_id).strip() if args.run_id is not None else str(int(time.time()))
    )
    ...
    engines, trt_build_meta = _maybe_build_trt(
        cfg, export_path, meta_path=trt_build_meta_path
    )
    ...
    static_seq_len = int(
        cfg.get("onnx", {}).get("static_seq_len", max_seq_len) or max_seq_len
    )
    static_batch = int(cfg.get("onnx", {}).get("static_batch", 1) or 1)
    ...
    trt_rebuild_on_init_fail = (
        os.getenv("BANTER_TR118_REBUILD_TRT_ON_INIT_FAIL", "1") != "0"
    )
    ...
    if export_path.exists():
        for backend_name, providers in [
            ...
            try:
                ...
                if (
                    backend_name.endswith("-gpu")
                    and "CUDAExecutionProvider" not in used
                ):
    ...
    def get_trt_runner(precision: str) -> TensorRTInferenceEngine | None:
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "Attempting TRT rebuild for %s due to init failure.", precision
                )
                try:
                    meta = build_tensorrt_engine(
                        ...
                        builder_settings=(
                            trt_builder_settings
                            if isinstance(trt_builder_settings, dict)
                            else None
                        ),
                    )
                    meta["built"] = True
                    ...
                    for i, r in enumerate(existing):
                        if (
                            ...
                            and str(r.get("output_path"))
                            == str(meta.get("output_path"))
                        ):
                            existing[i] = meta
                            replaced = True
                            break
                    ...
                    trt_build_meta_path.write_text(
                        json.dumps(existing, indent=2), encoding="utf-8"
                    )
                    ...
                    logger.warning(
                        "TRT rebuild failed for %s: %s", precision, rebuild_exc
                    )
    ...
    try:
        ...
        backend_init_meta["transformers"] = {
    ...
    def _default_torch_compile_backend() -> str | None:
        ...
        if override:
            return override
        configured = str(
            cfg.get("benchmark", {}).get("torch_compile_backend", "") or ""
        ).strip()
        if configured:
            return configured
    ...
    def get_torch_runner(
        device: str, compile_model: bool, batch_size: int, seq_len: int
    ) -> dict[str, Any] | None:
        ...
        start = time.perf_counter()
        model = torch_model_cls.from_pretrained(
            model_name, torch_dtype=torch_mod.float32
        )
        model.eval()
        model.to(device)
        init_ms = (time.perf_counter() - start) * 1000
        ...
        if compile_model and hasattr(torch_mod, "compile"):
            try:
                ...
                model = (
                    torch_mod.compile(  # type: ignore[attr-defined]
                        model, backend=compile_backend, dynamic=False
                    )
                    if compile_backend
                    else torch_mod.compile(  # type: ignore[attr-defined]
                        model, dynamic=False
                    )
                )
                ...
                Exception
            ) as exc:  # pragma: no cover - depends on platform/torch build
                compile_error = f"{type(exc).__name__}: {exc}"
                logger.warning(
                    "torch.compile failed; continuing without compile: %s",
                    compile_error,
                )
        runner_info = {
            "model": model,
    ...
    def run_one_backend_scenario(backend: str, scenario: str, rep: int) -> RunResult:
        ...
        prompt_set, prompts, batch_size = _resolve_scenario(
            scenario, prompt_sets, scenario_defs
        )
        if not prompts:
            return RunResult(
                spec=RunSpec(
        ...
        if not onnx_dynamic_axes:
            ...
            prompt_len = _compute_seq_len(
                tokenizer, prompts, max_len=max_seq_len, min_len=min_seq_len
            )
        ...
        with contextlib.ExitStack() as stack:
            ...
            if (
                enable_resource
                and ResourceMonitor is not None
                and resource_monitor_available
            ):
                try:
                    monitor = stack.enter_context(
                        ResourceMonitor(sample_interval_s=sample_interval)
                    )
                except (
                    Exception
                ) as exc:  # pragma: no cover - depends on host NVML/psutil
            ...
            if backend.startswith("tensorrt"):
                ...
                try:
                    prepared_trt = trt_runner.prepare(
                        batch=batch_size, seq=seq_len, vocab_size=vocab_size
                    )
                except Exception as exc:
                    logger.warning(
                        f"TensorRT prepare failed for {backend}: {exc}. Marking as degraded."
                    )
                    return RunResult(
                        spec=spec,
                        status="degraded",
                        error=f"tensorrt_prepare_failed:{exc!s}",
                        latencies_ms=[],
                        tokens_processed=[],
                        throughput_tok_s=[],
                        degraded_count=1,
                        degraded_reasons=[
                            f"tensorrt_prepare_error:{type(exc).__name__}"
                        ],
                        backend_metadata=backend_meta,
                    )
                ...
                try:
                    if mode == "generate":
                        if backend.startswith("onnxruntime"):
                            ...
                            def _infer_logits(
                                ids, mask, _sess=sess, _sess_info=sess_info
                            ):
                                return _infer_ort_logits(
                                    _sess,
                                    str(_sess_info.get("input_type", "int64")),
                                    ids,
                                    mask,
                                )

                            total_ms, ttft_ms, toks, out_text, is_deg, reason = (
                                _generate_uncached(
                                    _infer_logits,
                                    tokenizer,
                                    prompt,
                            ...
                            throughput.append(
                                toks / (total_ms / 1000.0) if total_ms > 0 else 0.0
                            )
                        ...
                        if backend.startswith("tensorrt"):
                            if trt_runner is None or prepared_trt is None:
                                ...
                                degraded_reasons.append(
                                    "trt_engine_missing_or_unavailable"
                                )
                                continue
                        ...
                        if backend.startswith("transformers"):
                            ...
                            def _infer_logits(
                                ids, mask, _runner=runner, _device=device
                            ):
                                ms, logits, is_deg, reason = _infer_torch_logits(
                                    _runner["model"], ids, mask, _device
                                )
                                if is_deg:
        ...
        if degraded > 0:
            status = "degraded"
            uniq = (
                ", ".join(sorted(set(degraded_reasons)))
                if degraded_reasons
                else "unknown"
            )
                ...
                for rep in range(repetitions):
                    try:
                        ...
                        result = RunResult(
                            ...
                            export_metadata=export_meta,
                            trt_build_metadata=trt_build_meta,
                            backend_metadata=backend_init_meta.get(backend)
                            or {"backend": backend},
                        )

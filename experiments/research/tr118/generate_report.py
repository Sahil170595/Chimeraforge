...
Consumes TR118 artifacts under `research/tr118/results/` and writes a Markdown
...
from research.shared.artifact_utils import resolve_repo_path
...
def _load_raw_jsonl(path: Path) -> pd.DataFrame:
    ...
    df["degraded"] = df.apply(
        lambda r: bool(r.get("degraded_count", 0) > 0)
        or str(r.get("status", "")) != "ok",
        axis=1,
    )
...
def _overall_backend_table(df: pd.DataFrame) -> str:
    df_ok = df[(df["run_mean_latency_ms"].notna()) & (not df["degraded"])]
    rows = []
    for backend, group in df.groupby("backend"):
        ...
        thr_vals = [
            float(v) for v in ok_group["run_mean_throughput_tok_s"].dropna().tolist()
        ]
...
def _overall_ttft_table(df: pd.DataFrame) -> str:
    df_ok = df[(df["run_mean_ttft_ms"].notna()) & (not df["degraded"])]
    rows = []
    for backend, group in df_ok.groupby("backend"):
        ...
        rows.append(
            [
                ...
                (
                    f"[{summary.ci_lower:.4g}, {summary.ci_upper:.4g}]"
                    if summary
                    else None
                ),
    ...
    return _md_table(
        ["Backend", "n_ok", "ttft_mean_ms", "ttft_ci95", "ttft_median_ms"], rows
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="research/tr118/configs/matrix_postdoc.yaml",
    ...
    manifest = (
        _read_json(manifest_path) if manifest_path and manifest_path.exists() else None
    )

    modes = args.modes
    if not modes:
        if manifest and isinstance(manifest.get("runs"), dict) and manifest.get("runs"):
            modes = [str(k) for k in manifest["runs"]]
        else:
            cfg_modes = cfg.get("benchmark", {}).get("modes")
            modes = (
                [str(m) for m in cfg_modes]
                if isinstance(cfg_modes, list) and cfg_modes
                else [str(cfg.get("benchmark", {}).get("mode", "prefill"))]
            )
    ...
    if manifest and manifest.get("prompt_config_path"):
        ...
        prompt_cfg = str(
            cfg.get("benchmark", {}).get(
                "prompt_config_path", "research/tr117/configs/matrix_tier3.yaml"
            )
        )
    ...
    if isinstance(manifest, dict) and manifest.get("perplexity_results_csv"):
        ppl_path = resolve_repo_path(
            _REPO_ROOT, str(manifest["perplexity_results_csv"])
        )

    ppl_json_path = processed_dir / "perplexity_results.json"
    if isinstance(manifest, dict) and manifest.get("perplexity_results_json"):
        ppl_json_path = resolve_repo_path(
            _REPO_ROOT, str(manifest["perplexity_results_json"])
        )

    export_meta_path = processed_dir / "export_metadata.json"
    if isinstance(manifest, dict) and manifest.get("export_metadata_path"):
        export_meta_path = resolve_repo_path(
            _REPO_ROOT, str(manifest["export_metadata_path"])
        )
    else:
        export_meta_path = (
            _latest_file(list(processed_dir.glob("export_metadata*.json")))
            or export_meta_path
        )

    trt_build_path = processed_dir / "trt_build_metadata.json"
    if isinstance(manifest, dict) and manifest.get("trt_build_metadata_path"):
        trt_build_path = resolve_repo_path(
            _REPO_ROOT, str(manifest["trt_build_metadata_path"])
        )
    else:
        trt_build_path = (
            _latest_file(list(processed_dir.glob("trt_build_metadata*.json")))
            or trt_build_path
        )
    ...
    for i, mode in enumerate(modes):
        ...
        if isinstance(runs, dict):
            run_entry = (
                runs.get(mode, {}) if isinstance(runs.get(mode, {}), dict) else {}
            )
        ...
        if raw_path is None:
            if args.raw and i == 0:
                ...
                raw_path = _latest_file(
                    list((results_dir / "raw").glob(f"bench_{mode}_*.jsonl"))
                )
        ...
        if isinstance(runs, dict):
            ...
            if run_entry.get("latency_summary_path"):
                summary_path = resolve_repo_path(
                    _REPO_ROOT, str(run_entry["latency_summary_path"])
                )
            if run_entry.get("statistical_analysis_path"):
                stats_path = resolve_repo_path(
                    _REPO_ROOT, str(run_entry["statistical_analysis_path"])
                )
        ...
        if df_raw is None:
            ...
            if "resource_metrics" in df_raw.columns:
                results_sections += [
                    ...
                    "",
                    _resource_summary_table(df_raw),
                    "",
                ]
    ...
    def _mode_baseline_latency(
        df: pd.DataFrame | None, baseline_backend: str
    ) -> float | None:
        if df is None or df.empty:
            return None
        df_ok = df[(df["run_mean_latency_ms"].notna()) & (not df["degraded"])]
    ...
    total_records = sum(
        int(df.shape[0]) for df in raw_by_mode.values() if df is not None
    )
    total_degraded = sum(
    ...
    all_ppl_pass = bool(
        ppl_df is not None
        and "pass" in ppl_df.columns
        and ppl_df["pass"].astype(bool).all()
    )
    ...
    if isinstance(trt_build_meta, list):
        for r in trt_build_meta:
            ...
            if str(r.get("precision", "")).lower() != "int8":
                continue
            eng = (
                r.get("engine_inspect")
                if isinstance(r.get("engine_inspect"), dict)
                else {}
            )
            insp = (
                eng.get("engine_inspector")
                if isinstance(eng.get("engine_inspector"), dict)
                else {}
            )
            ...
            calib = (
                r.get("int8_calibration")
                if isinstance(r.get("int8_calibration"), dict)
                else {}
            )
    ...
    if isinstance(ppl_json, dict):
        diag = (
            ppl_json.get("diagnostics")
            if isinstance(ppl_json.get("diagnostics"), dict)
            else {}
        )
        diffs = (
            diag.get("logit_diffs_last_token")
            if isinstance(diag.get("logit_diffs_last_token"), dict)
            else {}
        )
        int8 = (
            diffs.get("tensorrt-int8")
            if isinstance(diffs.get("tensorrt-int8"), dict)
            else None
        )
        if isinstance(int8, dict) and float(int8.get("max_abs", 0.0) or 0.0) == 0.0:
            sanity_warnings.append(
                "tensorrt-int8 last-token logit diff vs PyTorch is 0.0; suspect fallback/caching/measurement bug."
            )

    sanity_ok = not sanity_warnings
    if not missing and total_records > 0 and not any_degraded and all_ppl_pass:
        report_status = (
            "Complete (artifact-driven)"
            if sanity_ok
            else "Draft (sanity checks failed)"
        )
        meta_lines[3] = f"**Status:** {report_status}"

    baseline_backend = str(
        cfg.get("baseline", {}).get("backend", "transformers-gpu-compile")
    )
    degraded_rate = (
        (float(total_degraded) / float(total_records)) if total_records else 0.0
    )
    exec_lines: list[str] = [
        "## Executive Summary",
        "",
    ...
    if isinstance(manifest, dict):
        ...
        if isinstance(plat, dict):
            ...
            if isinstance(ort_providers, list) and ort_providers:
                env_lines.append(
                    f"- ONNXRuntime providers: {', '.join(str(p) for p in ort_providers)}"
                )
    ...
    if ppl_df is not None:
        ...
        results_sections += [
            ...
            "",
            _md_table(
                ["Backend", "PPL", "delta_frac", "Threshold", "Pass", "Error"], rows
            ),
            "",
        ]
    ...
    if export_meta or trt_build_meta or ppl_json:
        ...
        if export_meta:
            onnx_inspect = (
                export_meta.get("onnx_inspect")
                if isinstance(export_meta.get("onnx_inspect"), dict)
                else {}
            )
            weights = (
                export_meta.get("model_weight_files")
                if isinstance(export_meta.get("model_weight_files"), dict)
                else {}
            )
            sanity_sections += [
                ...
                _md_table(
                    ...
                    [
                        ...
                        [
                            "external_total_mb",
                            onnx_inspect.get("external_total_size_mb"),
                        ],
                        [
                            "total_artifact_mb",
                            onnx_inspect.get("total_artifact_size_mb"),
                        ],
                        [
                            "initializer_numel_est",
                            onnx_inspect.get("initializer_numel"),
                        ],
                        [
                            "initializer_bytes_est_mb",
                            onnx_inspect.get("initializer_bytes_est_mb"),
                        ],
            ...
            if (
                isinstance(weights.get("weight_files"), list)
                and weights["weight_files"]
            ):
                wf = []
        ...
        if isinstance(ppl_json, dict):
            ...
            base = (
                diag.get("baseline") if isinstance(diag.get("baseline"), dict) else {}
            )
            ref = (
                diag.get("reference_check_pytorch_loss")
                if isinstance(diag.get("reference_check_pytorch_loss"), dict)
                else {}
            )
            sanity_sections += [
                ...
                _md_table(
                    ...
                    [
                        ...
                        [
                            "expected_uniform_ppl",
                            diag.get("expected_uniform_perplexity"),
                        ],
                        [
                            "ref_loss_mean_nll",
                            ref.get("mean_nll") if isinstance(ref, dict) else None,
                        ],
                        [
                            "ref_loss_ppl",
                            ref.get("perplexity") if isinstance(ref, dict) else None,
                        ],
                    ],
                ),
                "",
        ...
        for r in trt_build_meta:
            rows.append(
                [
                    r.get("precision"),
                    (
                        Path(str(r.get("output_path"))).name
    ...
    reproducibility = [
        ...
        "",
        "```bash",
        f"python research/tr118/run_experiment.py --config {cfg_path} --device cuda",
        "```",
        "",
        "Generate this report from artifacts:",
        "",
        "```bash",
        f"python research/tr118/generate_report.py --config {cfg_path} --manifest {manifest_path or ''}".rstrip(),
        "```",
        "",
    ]

...
base_dir = Path("research/tr118/results/tr118v2_crossover")
...
for model in models:
    ...
    if prefill_files:
        with open(prefill_files[0], encoding="utf-8") as f:
            prefill_runs = len([line for line in f if line.strip()])

    if generate_files:
        with open(generate_files[0], encoding="utf-8") as f:
    ...
    print(
        f"  Processed: prefill={has_latency_prefill}, generate={has_latency_generate}, "
    ...
    if has_latency_prefill:
        ...
        with open(processed_dir / "latency_summary_prefill.csv", encoding="utf-8") as f:
            ...
            onnx_cpu = [
                r
                for r in rows
                if r["backend"] == "onnxruntime-cpu"
                and r["scenario"] == "single_medium"
            ]
            pytorch_gpu = [
                r
                for r in rows
            ...
            if onnx_cpu and pytorch_gpu:
                ...
                print(
                    f"  Crossover data (single_medium): ONNX CPU {onnx_tps:.0f} tok/s vs PyTorch GPU {pytorch_tps:.0f} tok/s = {advantage:.2f}x"
                )

...
class TR118v21Generator:
    ...
    def load_data(self):
        ...
        print(
            f"  Loaded: {len(self.tiny_prefill) + len(self.tiny_generate) + len(self.gpt2_prefill) + len(self.gpt2_generate)} data rows"
        )
    ...
    def build_executive_summary(self):
        ...
        tiny_ort_cpu = self.tiny_prefill[
            self.tiny_prefill["backend"] == "onnxruntime-cpu"
        ]["throughput_mean_tok_s"].mean()
        tiny_pytorch = self.tiny_prefill[
            self.tiny_prefill["backend"] == "transformers-gpu-compile"
        ]["throughput_mean_tok_s"].mean()
        gpt2_ort_cpu = self.gpt2_prefill[
            self.gpt2_prefill["backend"] == "onnxruntime-cpu"
        ]["throughput_mean_tok_s"].mean()
        gpt2_pytorch = self.gpt2_prefill[
            self.gpt2_prefill["backend"] == "transformers-gpu-compile"
        ]["throughput_mean_tok_s"].mean()
        ...
        self.add(
            "## Executive Summary",
            "",
            "Through 720 benchmark runs (360 prefill + 360 generate), we analyze ONNX Runtime and TensorRT performance scaling across 1,210x parameter increase (0.103M → 124.4M).",
            "",
            "### Key Findings (All numbers are overall means across 6 scenarios)",
            "",
            f"1. **The Crossover Phenomenon:** ONNX CPU goes from **{crossover_tiny:.1f}x faster** (tiny, {tiny_ort_cpu:.0f} vs {tiny_pytorch:.0f} tok/s) to **{crossover_gpt2:.2f}x slower** (gpt2, {gpt2_ort_cpu:.0f} vs {gpt2_pytorch:.0f} tok/s)",
            "",
            "2. **TensorRT Scales Perfectly:** Maintains 60-75% speedup across 1,210x parameter increase",
            "",
    ...
    def add_prefill_table(self, df: pd.DataFrame, _model: str):
        ...
        baseline = df[df["backend"] == "transformers-gpu-compile"][
    ...
    def add_generate_table(
        self, df: pd.DataFrame, _model: str, mark_trt_degraded=False
    ):
        ...
        if mark_trt_degraded:
            ...
            df_valid_sorted = df_valid.sort_values(
                "throughput_mean_tok_s", ascending=False
            )
    ...
    def build_conclusions(self):
        ...
        self.add(
            "## Conclusions",
            "",
            "1. **ONNX CPU collapses at scale:** 21.9x faster → 0.68x slower as model grows 1,210x",
    ...
    def build_reproducibility(self):
        ...
        self.add(
            ...
            "",
            "**Artifacts:**",
            "- Raw JSONL: `research/tr118/results/tr118v2/20251213_135135_deep/{model}/raw/`",
            "- Processed CSV: `research/tr118/results/tr118v2/20251213_135135_deep/{model}/processed/`",
    ...
    def generate(self):
        ...
        newline = "\n"
        print(f"     Size: {len(newline.join(self.report_lines))} bytes")

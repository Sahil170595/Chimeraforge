...
class TR118v2DataExtractor:
    ...
    def get_param_counts(self) -> dict[str, int]:
        ...
        return {"tiny-gpt2": 102714, "gpt2": 124439808}
    ...
    def get_degraded_status(self, model: str, mode: str) -> dict[str, dict]:
        ...
        model_path = self.tiny_path if model == "tiny-gpt2" else self.gpt2_path
        jsonl_file = (
            f"bench_{mode}_1765651895.jsonl"
            if model == "tiny-gpt2"
            else f"bench_{mode}_1765652089.jsonl"
        )
...
class TR118v2ReportBuilder:
    ...
    def build_executive_summary(self):
        ...
        tiny_ort_cpu = tiny_prefill[tiny_prefill["backend"] == "onnxruntime-cpu"][
            "throughput_mean_tok_s"
        ].mean()
        tiny_pytorch = tiny_prefill[
            tiny_prefill["backend"] == "transformers-gpu-compile"
        ]["throughput_mean_tok_s"].mean()
        gpt2_ort_cpu = gpt2_prefill[gpt2_prefill["backend"] == "onnxruntime-cpu"][
            "throughput_mean_tok_s"
        ].mean()
        gpt2_pytorch = gpt2_prefill[
            gpt2_prefill["backend"] == "transformers-gpu-compile"
        ]["throughput_mean_tok_s"].mean()
        ...
        self.add(
            ...
            "",
            f"1. **The Crossover Phenomenon:** ONNX CPU goes from **{crossover_tiny:.1f}x faster** (tiny-gpt2, {tiny_ort_cpu:.0f} vs {tiny_pytorch:.0f} tok/s) to **{crossover_gpt2:.2f}x slower** (gpt2, {gpt2_ort_cpu:.0f} vs {gpt2_pytorch:.0f} tok/s) - a {degradation:.0f}x degradation in relative advantage",
            "",
            "2. **TensorRT Perfect Scaling:** Maintains consistent 60-75% speedup across 1,210x parameter increase (prefill mode)",
            "",

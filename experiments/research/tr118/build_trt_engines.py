...
def _build_int8_calibration_batches(
    ...
    try:
        ...
        logger.warning(
            "Tokenizer unavailable for calibration; using random tokens: %s", exc
        )
        tokenizer = None
    ...
    if tokenizer is not None:
        try:
            ...
            logger.warning(
                "datasets unavailable for calibration; using random tokens: %s", exc
            )
            texts = []
    ...
    if texts and tokenizer is not None:
        ...
        vocab = (
            int(getattr(tokenizer, "vocab_size", 50257) or 50257)
            if tokenizer is not None
            else 50257
        )
...
def build_tensorrt_engine(
    ...
    profiles: (
        list[dict[str, tuple[tuple[int, ...], tuple[int, ...], tuple[int, ...]]]] | None
    ) = None,
    ...
    if precision == "int8" and int8_calibration is not None:
        ...
        try:
            from research.tr118.trt_inference import ensure_cuda_dll_search_path
    ...
    if isinstance(builder_settings, dict) and builder_settings:
        try:
            if "builder_optimization_level" in builder_settings and hasattr(
                config, "builder_optimization_level"
            ):
                config.builder_optimization_level = int(
                    builder_settings["builder_optimization_level"]
                )
            if "max_num_tactics" in builder_settings and hasattr(
                config, "max_num_tactics"
            ):
                config.max_num_tactics = int(builder_settings["max_num_tactics"])
            if "tiling_optimization_level" in builder_settings and hasattr(
                config, "tiling_optimization_level"
            ):
                config.tiling_optimization_level = int(
                    builder_settings["tiling_optimization_level"]
                )
            if "profiling_verbosity" in builder_settings and hasattr(
            ...
            if "tactic_sources" in builder_settings and hasattr(
                ...
                if isinstance(sources, list) and sources:
                    ...
                    for s in sources:
                        ...
                        if not hasattr(trt.TacticSource, name):
                            continue
                        tactic_mask |= 1 << int(getattr(trt.TacticSource, name))
                    if tactic_mask:
                        config.set_tactic_sources(int(tactic_mask))
        ...
        if not builder.platform_has_fast_int8:
            ...
            if int8_calibration is not None:
                ...
                cache_path = resolve_repo_path(
                    _REPO_ROOT,
                    str(
                        int8_calibration.get("cache_path", str(output_path) + ".calib")
                    ),
                )
                cache_hit_before = bool(cache_path.exists())
                cache_size_before = (
                    int(cache_path.stat().st_size) if cache_path.exists() else None
                )
                ...
                class _EntropyCalibrator(trt.IInt8EntropyCalibrator2):  # type: ignore[misc]
                    ...
                    def get_batch(self, names):  # type: ignore[override]
                        ...
                        self._i += 1
                        cuda.memcpy_htod(
                            self._d_inputs["input_ids"], batch["input_ids"]
                        )
                        cuda.memcpy_htod(
                ...
                logger.warning(
                    "INT8 enabled but no calibrator set (allow_no_calibrator=1)"
                )
    elif precision != "fp32":
    ...
    if profiles:
        for idx, profile_shapes in enumerate(profiles):
            ...
            logger.info(
                "Added optimization profile %d with %d inputs", idx, len(profile_shapes)
            )
    elif dynamic_shapes:
        profile = builder.create_optimization_profile()
    ...
    if (
        ...
        and calibration_shape is not None
        and added_profiles
    ):
        ...
        if profiles:
            for idx, profile_shapes in enumerate(profiles):
                ...
                mask_shapes = profile_shapes.get("attention_mask")
                if not ids or not mask_shapes:
                    continue
                min_ids, _opt_ids, max_ids = ids
                min_mask, _opt_mask, max_mask = mask_shapes
                if (
            ...
            mask_shapes = dynamic_shapes.get("attention_mask")
            if ids and mask_shapes:
                min_ids, _opt_ids, max_ids = ids
                min_mask, _opt_mask, max_mask = mask_shapes
                if (
                    min_ids[0] <= calib_batch <= max_ids[0]
        ...
        if selected is not None and 0 <= selected < len(added_profiles):
            ...
            logger.info(
                "Set INT8 calibration profile to %d (%dx%d)",
                selected,
                calib_batch,
                calib_seq,
            )
            ...
            logger.info(
                "Added INT8 calibration profile (%dx%d)", calib_batch, calib_seq
            )
    ...
    with output_path.open("wb") as engine_file:
        engine_file.write(serialized_engine)

    file_size_mb = output_path.stat().st_size / (1024 * 1024)
    logger.info(
        f"Engine saved: {output_path} ({file_size_mb:.2f} MB) in {build_time:.2f}s"
    )
    ...
    for key in (
        ...
        if hasattr(config, key):
            try:
                ...
                applied_builder_settings[key] = (
                    int(val) if isinstance(val, (int,)) else str(val)
                )
            except Exception:
...
def validate_trt_inference(
    ...
    latency_ms, logits = runner.infer(
        prepared, test_inputs["input_ids"], test_inputs["attention_mask"]
    )
...
def main():
    ...
    parser.add_argument(
        "--workspace-gb", type=int, default=4, help="Max workspace size in GB"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Run validation inference"
    )
    ...
    if args.onnx:
        ...
        onnx_path = (
            resolve_repo_path(_REPO_ROOT, str(cfg["output"]["onnx_dir"]))
            / f"{Path(model_name).name}.onnx"
        )
    else:
    ...
    if precisions is None:
        precisions = (
            list(cfg.get("tensorrt", {}).get("precisions", ["fp16"]))
            if cfg
            else ["fp16"]
        )
    ...
    if cfg is not None:
        ...
        if isinstance(int8_calibration, dict) and int8_calibration.get("cache_path"):
            ...
            int8_calibration["cache_path"] = str(
                resolve_repo_path(_REPO_ROOT, str(int8_calibration["cache_path"]))
            )
        ...
        try:
            ...
            if output_path.exists():
                try:
                    ...
                    logger.warning(
                        f"Failed to clean up partial artifact: {cleanup_exc}"
                    )
    ...
    logger.info(f"\n{'='*60}")
    logger.info(
        f"BUILD SUMMARY: {successful}/{len(precisions)} engines built successfully"
    )
    logger.info(f"{'='*60}")

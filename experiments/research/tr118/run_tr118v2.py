...
`research/tr118/run_experiment.py`.
    ...
    try:
        return (
            subprocess.check_output(
                ["git", "rev-parse", "HEAD"], text=True, cwd=str(_REPO_ROOT)
            ).strip()
            or None
        )
    except Exception:
...
def main() -> int:
    ...
    parser.add_argument(
        "--results-root",
        default="research/tr118/results/tr118v2",
    ...
    parser.add_argument(
        "--tiny-model",
        default="models/tiny-gpt2",
        help="Tiny model name/path (repo-local by default)",
    )
    ...
    parser.add_argument(
        "--skip-accuracy", action="store_true", help="Skip perplexity validation"
    )
    parser.add_argument("--with-plots", action="store_true", help="Generate plots")
    parser.add_argument(
        "--with-report",
        action="store_true",
        help="Generate per-run TR118 reports into each run folder",
    )
    parser.add_argument(
        "--force-export",
        action="store_true",
        help="Force ONNX re-export for the first mode",
    )
    parser.add_argument(
        "--force-trt-rebuild",
        action="store_true",
        help="Force TRT rebuild for the first mode",
    )
    ...
    for key, model_name, desc in model_runs:
        ...
        trt_cfg = (
            base_cfg.get("tensorrt")
            if isinstance(base_cfg.get("tensorrt"), dict)
            else {}
        )
        int8_cfg = (
            trt_cfg.get("int8_calibration")
            if isinstance(trt_cfg.get("int8_calibration"), dict)
            else None
        )
        if int8_cfg is not None:
            int8_cfg = dict(int8_cfg)
            int8_cfg["cache_path"] = str(
                _make_int8_cache_path(artifacts_root, model_name, int8_cfg).as_posix()
            )
        ...
        manifest = (
            _latest_file(list(proc_dir.glob("experiment_manifest_*.json")))
            if success
            else None
        )
        tr118v2_runs.append(

...
models = {
    "50m": Path(
        "research/tr118/results/tr118v2_crossover/gpt2-50m/processed/latency_summary_prefill.csv"
    ),
    "75m": Path(
        "research/tr118/results/tr118v2_crossover/gpt2-75m/processed/latency_summary_prefill.csv"
    ),
    "100m": Path(
        "research/tr118/results/tr118v2_crossover/gpt2-100m/processed/latency_summary_prefill.csv"
    ),
}
...
for name, path in models.items():
    with open(path, encoding="utf-8") as f:
        ...
        onnx_cpu = next(
            r
            for r in rows
            if r["backend"] == "onnxruntime-cpu" and r["scenario"] == "single_medium"
        )
        pytorch_gpu = next(
            r
            for r in rows
        ...
        print(
            f"  Advantage: {advantage:.2f}x ({'ONNX faster' if advantage > 1 else 'PyTorch faster'})"
        )

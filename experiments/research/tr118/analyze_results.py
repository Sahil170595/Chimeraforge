...
def main() -> int:
    ...
    if args.raw:
        ...
        if not raw_files:
            raw_files = sorted(
                raw_dir.glob("bench_*.jsonl"), key=lambda p: p.stat().st_mtime
            )
            raw_files = [p for p in raw_files if p.exists() and p.stat().st_size > 0]
    ...
    totals["degraded_rate"] = totals["n_degraded"] / totals["n_total"].where(
        totals["n_total"] > 0, 1
    )
    ...
    for r in totals.to_dict(orient="records"):
        ...
        thr_values = (
            [float(v) for v in group["run_mean_throughput_tok_s"].dropna().tolist()]
            if group is not None
            else []
        )
        ...
        ttft_values = (
            [float(v) for v in group["run_mean_ttft_ms"].dropna().tolist()]
            if group is not None
            else []
        )
        ...
        warm_values = (
            [float(v) for v in group["run_mean_warmup_latency_ms"].dropna().tolist()]
            if group is not None
            else []
        )
    ...
    if baseline_backend and baseline_backend in set(df_ok["backend"]):
        ...
        base_ttft_all = [
            float(v) for v in base_df["run_mean_ttft_ms"].dropna().tolist()
        ]
        if base_ttft_all:
            for backend, group in df_ok.groupby("backend"):
                if backend == baseline_backend:
        ...
        for scenario in sorted(set(df_ok["scenario"].tolist())):
            ...
            base_thr = [
                float(v) for v in base_s["run_mean_throughput_tok_s"].dropna().tolist()
            ]
            base_ttft = [float(v) for v in base_s["run_mean_ttft_ms"].dropna().tolist()]
            for backend, group in df_ok[df_ok["scenario"] == scenario].groupby(
                "backend"
            ):
                if backend == baseline_backend:
                    continue
                ...
                thr_vals = [
                    float(v)
                    for v in group["run_mean_throughput_tok_s"].dropna().tolist()
                ]

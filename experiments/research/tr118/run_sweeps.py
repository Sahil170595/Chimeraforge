...
def _apply_default_isolation(cfg: dict[str, Any], variant_name: str) -> dict[str, Any]:
    ...
    output["results_dir"] = f"research/tr118/results/sweeps/{variant_name}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--base-config",
        default="research/tr118/configs/sweep_base.yaml",
        help="Base TR118 config yaml",
    )
    parser.add_argument(
        "--variants-dir",
        default="research/tr118/configs/variants",
    ...
    def _run_variant(name: str, override: dict[str, Any]) -> None:
        ...
        results_dir = resolve_repo_path(
            _REPO_ROOT, str(merged.get("output", {}).get("results_dir", ""))
        )
        ...
        if not args.with_report:
            ...
            cmd += [
                "--report-out",
                str(processed_dir / f"Technical_Report_118_{name}.md"),
            ]
    ...
    out_root = resolve_repo_path(_REPO_ROOT, "research/tr118/results/sweeps")

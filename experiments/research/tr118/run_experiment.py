...
Artifacts are written under `research/tr118/results/` as configured.
"""
...
def _try_version(module_name: str) -> str | None:
    try:
        ...
        return None
    return str(
        getattr(mod, "__version__", None) or getattr(mod, "version", None) or None
    )
...
def _latest_jsonl_for_mode(results_dir: Path, mode: str) -> Path | None:
    raw_dir = results_dir / "raw"
    files = sorted(
        raw_dir.glob(f"bench_{mode}_*.jsonl"), key=lambda p: p.stat().st_mtime
    )
    return files[-1] if files else None
...
def main() -> int:
    ...
    for idx, mode in enumerate(modes):
        ...
        _run(
            [
                sys.executable,
                "research/tr118/run_benchmark.py",
                "--config",
                str(cfg_path),
                "--prompt-config",
                str(prompt_cfg_path),
        ...
        if not raw_path.exists():
            raw_path = _latest_jsonl_for_mode(results_dir, str(mode)) or _latest_jsonl(
                results_dir
            )
        runs[str(mode)] = {
            ...
            "statistical_analysis_path": str(
                processed_dir / f"statistical_analysis_{mode}.json"
            ),
        }

        analyze_cmd = [
            sys.executable,
            "research/tr118/analyze_results.py",
            "--config",
            str(cfg_path),
            "--mode",
            str(mode),
    ...
    manifest["runs"] = runs
    manifest["export_metadata_path"] = str(
        processed_dir / f"export_metadata_{run_id}.json"
    )
    manifest["trt_build_metadata_path"] = str(
        processed_dir / f"trt_build_metadata_{run_id}.json"
    )
    ...
    if not args.skip_accuracy:
        _run(
            [
                sys.executable,
                "research/tr118/validate_accuracy.py",
                "--config",
                str(cfg_path),
                "--device",
                str(args.device),
        ...
        manifest["perplexity_results_csv"] = str(
            processed_dir / "perplexity_results.csv"
        )
        manifest["perplexity_results_json"] = str(
            processed_dir / "perplexity_results.json"
        )
    ...
    if not args.no_report:
        try:
            ...
            _run(
                [
                    sys.executable,
                    "research/tr118/generate_report.py",
                    "--config",
                    str(cfg_path),
                    "--output",
                    str(report_out),

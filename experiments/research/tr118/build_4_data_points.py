...
def estimate_params(
    n_layer: int, n_embd: int, n_inner: int, vocab_size: int = 50257, _n_head: int = 1
) -> int:
    ...
    return embed_params + pos_params + layer_params + final_ln
...
def find_config_for_target(target_params: float, tolerance: float = 0.2) -> GPT2Config:
            ...
            for n_head in [2, 4, 8, 12, 16]:
                ...
                try:
                    params = estimate_params(
                        n_layer, n_embd, n_inner, vocab_size, n_head
                    )
    ...
    if best_config is None:
        raise ValueError(
            f"Could not find config for {target_params}M params (tried tolerance {tolerance*100}%)"
        )
...
def build_model(
    target_params: float, model_name: str
) -> tuple[GPT2LMHeadModel, GPT2Config, Path]:
...
def main():
    ...
    for target_params, model_name in targets:
        ...
        if (
            output_dir.exists()
            and (output_dir / "config.json").exists()
        ...
        try:
            model, _config, output_dir = build_model(target_params, model_name)
    ...
    if len(models_built) == len(targets):
        ...
        for _model_name, output_dir in models_built:
            print(
                f"    python research/tr118/run_tr118v2.py --device cuda --models custom --model-path {output_dir}"
            )
    else:

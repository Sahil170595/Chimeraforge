"""TR135: LLM judge post-hoc analysis for concurrency safety samples.

Reads samples.jsonl, runs LLM-as-judge on safety tasks, writes tr135_judged.jsonl.

Usage:
    python research/tr135/judge_analysis.py [-v] [--run-dir PATH] [--limit N]
"""
...
from research.tr135.shared.utils import SAFETY_TASKS, find_latest_run

logger = logging.getLogger("tr135.judge_analysis")
...
def run_judge(run_dir: Path, limit: int = 0) -> int:
    ...
    output_path = run_dir / "tr135_judged.jsonl"
    judged = 0
    parse_failures = 0
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for i, rec in enumerate(safety_records):
            ...
            try:
                ...
                logger.warning("Judge failed for %s/%s: %s", rec["model"], rec.get("sample_id"), e)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="TR135 LLM judge")

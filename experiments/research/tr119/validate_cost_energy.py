...
Outputs:
- `research/tr119/results/processed/cost_energy_validation.json`
"""
...
def validate_cost_model(
    ...
    on_demand = float(
        config.get("cloud_pricing", {}).get("on_demand_usd_per_hour", 1.0)
    )
    usd_per_kwh = float(config.get("energy", {}).get("usd_per_kwh", 0.2))
    require_cpu_power = bool(
        config.get("telemetry", {}).get("require_cpu_power", False)
    )
    ...
    for row in rows:
        ...
        infra_keys = [
            k for k in row if str(k).startswith("infra_cost_usd_per_1m_tokens_")
        ]
        total_keys = [
            k for k in row if str(k).startswith("total_cost_usd_per_1m_tokens_")
        ]
        ...
        for infra_key in infra_keys:
            ...
            if infra_cost > 1000.0:
                warnings.append(
                    f"{label}: very high {infra_key} ({infra_cost:.2f} USD/1M tokens)"
                )
        ...
        for total_key in total_keys:
            ...
            if total_cost > 2000.0:
                warnings.append(
                    f"{label}: very high {total_key} ({total_cost:.2f} USD/1M tokens)"
                )
    ...
    for _, row in latency_summary.iterrows():
        ...
        if power is not None:
            ...
            if power < 5.0 and "gpu" in backend.lower():
                warnings.append(
                    f"{backend}: suspiciously low power ({power:.1f}W) for GPU backend"
                )
...
def main() -> int:
    ...
    cost_summary = (
        json.loads(cost_summary_path.read_text(encoding="utf-8"))
    ...
    latency_summary = (
        pd.read_csv(latency_summary_path)

...
Artifacts are written under `research/tr119/results/` as configured.
"""
...
from research.shared.artifact_utils import resolve_repo_path  # reuse helper
...
def main() -> int:
    ...
    bench_cmd = [
        sys.executable,
        "research/tr119/run_benchmark.py",
        "--config",
        str(cfg_path),
        "--prompt-config",
        str(prompt_cfg_path),
    ...
    _run(
        [
            sys.executable,
            "research/tr119/analyze_results.py",
            "--config",
            str(cfg_path),
        ]
    )
    manifest["latency_summary_path"] = str(processed_dir / "latency_summary_cost.csv")
    manifest["cost_energy_summary_path"] = str(
        processed_dir / "cost_energy_summary.json"
    )
    ...
    try:
        _run(
            [
                sys.executable,
                "research/tr119/statistical_analysis.py",
                "--cost-summary",
                str(processed_dir / "cost_energy_summary.json"),
            ]
        )
        manifest["statistical_analysis"] = str(
            processed_dir / "statistical_analysis.json"
        )
    ...
    if not args.skip_validation:
        try:
            _run(
                [
                    sys.executable,
                    "research/tr119/validate_cost_energy.py",
                    "--config",
                    str(cfg_path),
                ]
            )
            manifest["cost_energy_validation"] = str(
                processed_dir / "cost_energy_validation.json"
            )
        except subprocess.CalledProcessError as exc:
    ...
    if not args.skip_deep_analysis:
        try:
            _run(
                [
                    sys.executable,
                    "research/tr119/cost_energy_analysis.py",
                    "--config",
                    str(cfg_path),
                ]
            )
            manifest["cost_energy_analysis"] = str(
                processed_dir / "cost_energy_analysis.json"
            )
        except subprocess.CalledProcessError as exc:
    ...
    if not args.no_report:
        try:
            ...
            _run(
                [
                    sys.executable,
                    "research/tr119/generate_report.py",
                    "--config",
                    str(cfg_path),
                    "--output",
                    str(report_out),

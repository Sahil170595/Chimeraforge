...
def _apply_default_isolation(cfg: dict[str, Any], variant_name: str) -> dict[str, Any]:
    ...
    output["results_dir"] = f"research/tr119/results/sweeps/{variant_name}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--variants-dir",
        default="research/tr119/configs/variants",
    ...
    for variant_name, variant_path in variants_to_run:
        ...
        results_root = resolve_repo_path(
            _REPO_ROOT, str(merged_cfg["output"]["results_dir"])
        )
        processed_dir = results_root / "processed"
        processed_dir.mkdir(parents=True, exist_ok=True)
        merged_cfg_path = (
            processed_dir / f"merged_config_{variant_name}_{sweep_id}.yaml"
        )
        merged_cfg_path.write_text(
            yaml.dump(merged_cfg, default_flow_style=False), encoding="utf-8"
        )

        variant_started = time.time()
        cmd = [
            sys.executable,
            "research/tr119/run_experiment.py",
            "--config",
            str(merged_cfg_path),
            "--device",
            str(args.device),
    ...
    sweep_results_root = resolve_repo_path(_REPO_ROOT, "research/tr119/results/sweeps")

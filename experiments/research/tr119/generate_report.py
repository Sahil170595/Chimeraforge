...
Consumes TR119 artifacts under `research/tr119/results/` and writes a Markdown
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="research/tr119/configs/baseline.yaml",
    ...
    if args.manifest:
        ...
        manifests = sorted(
            processed_dir.glob("experiment_manifest_*.json"),
    ...
    cpu_power_model = (
        telemetry_cfg.get("cpu_power_model", {})
    ...
    if (
        analysis_cfg.get("prompt_tokens") is not None
        or analysis_cfg.get("generate_tokens") is not None
    ):
        methodology += [
            "### Request Token Mix",
            "",
    ...
    if manifest:
        ...
        cuda_dev = (
            plat.get("cuda_device") if isinstance(plat.get("cuda_device"), dict) else {}
        )
        if cuda_dev.get("name"):
            ...
            if isinstance(mem_mb, (int, float)):
                ...
                exp_design.append(
                    f"- GPU: {cuda_dev.get('name')} (CC {cuda_dev.get('capability', 'N/A')})"
                )
        exp_design.append("")
        ...
        if "mode" in df_latency.columns:
            ...
            if modes:
                ...
                if not updated:
                    ...
                    exp_design.append("")
    cost_data = (
        json.loads(cost_json.read_text(encoding="utf-8"))
        if cost_json.exists()
        else None
    )
    ...
    if cost_data and isinstance(cost_data.get("rows"), list) and cost_data["rows"]:
        ...
        if "total_cost_usd_per_1m_tokens_spot" in df_cost.columns:
            spot_agg = (
                df_cost_mode.groupby("backend")["total_cost_usd_per_1m_tokens_spot"]
                .mean()
                .sort_values()
            )
            best_spot = float(spot_agg.iloc[0])
            savings_pct = (
                ((best_cost - best_spot) / best_cost * 100) if best_cost > 0 else 0.0
            )
            exec_lines += [
                f"- Best spot pricing ({mode_label}): **{spot_agg.index[0]!s}** at ~**${best_spot:.4g} per 1M tokens** ({savings_pct:.1f}% savings vs on-demand).",
            ]

        # Carbon footprint if available
        if "carbon_gco2e_per_1m_tokens" in df_cost.columns:
            carbon_agg = (
                df_cost_mode.groupby("backend")["carbon_gco2e_per_1m_tokens"]
                .mean()
                .sort_values()
            )
        ...
        if "energy_kwh_per_1m_tokens" in df_cost.columns:
            ...
            eff = (
                eff_df.groupby("backend")["tokens_per_kwh"]
                .mean()
                .sort_values(ascending=False)
            )
            if not eff.empty:
                exec_lines += [
                    f"- Best energy efficiency ({mode_label}): **{eff.index[0]!s}** at ~**{eff.iloc[0]:.0f} tokens/kWh**.",
                ]
        ...
        if isinstance(providers, dict) and providers:
            ...
            for provider, pricing in providers.items():
                ...
                for backend in df_cost_mode["backend"].unique():
                    ...
                    total_cost = gpu_hours * float(on_demand) + (
                        energy_kwh * energy_usd
                    )
                ...
                if request_costs:
                    best_req = min(
                        ...
                        key=lambda x: x[1].get(
                            "total_cost_usd_per_request", float("inf")
                        ),
                    )
        ...
        exec_lines += [
            "- Cost/energy artifacts missing; this draft focuses on latency only.",
            "",
        ]
    ...
    if df_latency is not None:
        if "n_total" in df_latency.columns:
            ...
            degraded_runs = (
                int(df_latency["n_degraded"].sum())
                if "n_degraded" in df_latency.columns
                else 0
            )
    ...
    if df_latency is not None:
        ...
        for _, r in lat_agg.iterrows():
            ...
            power_mean = (
                r.get("cpu_power_mean_watts")
                if is_cpu_backend
                else r.get("gpu_power_mean_watts")
            )
            if not isinstance(power_mean, (int, float)) or pd.isna(power_mean):
                power_mean = (
                    r.get("gpu_power_mean_watts")
                    if is_cpu_backend
                    else r.get("cpu_power_mean_watts")
                )
            lat_agg_rows.append(
                [
                    backend,
        ...
        for r in df_latency.to_dict(orient="records"):
            ...
            if power_mean is None or (
                isinstance(power_mean, float) and pd.isna(power_mean)
            ):
    ...
    if cost_data and isinstance(cost_data.get("rows"), list):
        ...
        cost_agg = (
            ...
            .agg(
                total_cost_usd_per_1m=(
                    ("total_cost_usd_per_1m_tokens_on_demand", "mean")
                    if "total_cost_usd_per_1m_tokens_on_demand" in df_cost.columns
        ...
        for r in df_cost.to_dict(orient="records"):
            ...
            rows_cost.append(
                [
                    ...
                    r.get("infra_cost_usd_per_1m_tokens_on_demand")
                    or r.get("infra_cost_usd_per_1m_tokens"),
                    r.get("energy_cost_usd_per_1m_tokens"),
                    r.get("total_cost_usd_per_1m_tokens_on_demand")
                    or r.get("total_cost_usd_per_1m_tokens"),
                ]
            )
        results_sections += [
        ...
        tier_cols = [c for c in tier_labels if c in df_cost.columns]
        if (
            len(tier_cols) >= 2
            and "total_cost_usd_per_1m_tokens_on_demand" in tier_cols
        ):
            rows_tier: list[list[Any]] = []
            for r in df_cost.to_dict(orient="records"):
                rows_tier.append(
                    [r.get("backend"), r.get("mode"), r.get("scenario")]
                    + [r.get(c) for c in tier_cols]
                )
            results_sections += [
                ...
                _md_table(
                    ...
                    + [tier_labels[c] for c in tier_cols],
                    rows_tier,
                ),
                "",
            ]
        ...
        if isinstance(providers, dict) and providers:
            ...
            for provider, pricing in providers.items():
                ...
                if reserved is None:
                    reserved = pricing.get("reserved_1yr_usd_per_hour") or pricing.get(
                        "reserved_3yr_usd_per_hour"
                    )
                if on_demand is None:
                    continue
                for (backend, mode), subset in df_cost.groupby(["backend", "mode"]):
                    gpu_hours = float(subset["gpu_hours_per_1m_tokens"].mean())
                    energy_kwh = float(subset["energy_kwh_per_1m_tokens"].mean())
                    on_demand_cost = gpu_hours * float(on_demand) + (
                        energy_kwh * energy_usd
                    )
                    spot_cost = (
    ...
    if (
        cost_data
        and isinstance(cost_data.get("rows"), list)
        and "df_cost" in locals()
        and isinstance(df_cost, pd.DataFrame)
    ):
        try:
            cost_col_dd = (
                "total_cost_usd_per_1m_tokens_on_demand"
            ...
            if "mode" in df_cost.columns and cost_col_dd in df_cost.columns:
                ...
                if not pre.empty:
                    ...
                    delta_pre = (
                        ((second_pre / best_pre - 1.0) * 100.0)
                        if (second_pre and best_pre > 0)
                        else None
                    )
                    ...
                    best_pre_energy_pct = (
                        float(pre_stats.loc[best_pre_b]["energy_cost"])
                        / float(pre_stats.loc[best_pre_b]["total_cost"])
                        * 100.0
    ...
    if analysis_json.exists():
        try:
            ...
            if "cost_breakdown" in analysis_data and isinstance(
                analysis_data["cost_breakdown"], dict
            ):
            ...
            if "energy_efficiency_ranking" in analysis_data:
                ...
                for backend, tokens_per_kwh in sorted(
                    eff_ranking.items(), key=lambda x: x[1], reverse=True
                ):
                    results_sections.append(
                        f"- **{backend}**: {tokens_per_kwh:.0f} tokens/kWh"
                    )
                results_sections.append("")
            ...
            if "roi_by_tier" in analysis_data:
                ...
                for backend, data in sorted(
                    roi.items(),
                    key=lambda x: x[1].get("spot_savings_pct") or 0,
                    reverse=True,
                ):
                    ...
                    spot_fmt = (
                        f"{spot_savings:.1f}%"
                        if isinstance(spot_savings, (int, float))
                        else "N/A"
                    )
                    reserved_fmt = (
                        f"{reserved_savings:.1f}%"
                        if isinstance(reserved_savings, (int, float))
                        else "N/A"
                    )
                    results_sections.append(
            ...
            if analysis_data.get("request_costs"):
            ...
            if isinstance(by_mode, dict) and by_mode:
                for mode, data in by_mode.items():
                    ...
                    if data.get("anova", {}).get("significant"):
                        ...
                        stat_section.append(
                            "- **ANOVA**: No significant differences detected across backends"
                        )
                    stat_section.append("")
                    if data.get("pairwise_comparisons"):
                        sig_comps = [
                            c
                            for c in data["pairwise_comparisons"]
                ...
                if data.get("pairwise_comparisons"):
                    sig_comps = [
                        c for c in data["pairwise_comparisons"] if c.get("significant")
                    ]
                    if sig_comps:
                        stat_section.append(
                            "### Significant Pairwise Comparisons (p < 0.05)"
    ...
    if cost_data and isinstance(cost_data.get("rows"), list) and cost_data["rows"]:
        try:
            ...
            if "mode" in df_dm.columns and cost_col_dm in df_dm.columns:
                pivot = (
                    df_dm.groupby(["backend", "mode"])[cost_col_dm]
                    .mean()
                    .unstack("mode")
                )
                ...
                for backend in sorted(pivot.index.astype(str).tolist()):
                    pre = (
                        pivot.loc[backend].get("prefill")
                        if "prefill" in pivot.columns
                        else None
                    )
                    gen = (
                        pivot.loc[backend].get("generate")
                        if "generate" in pivot.columns
                        else None
                    )
                    ratio = (
                        (float(gen) / float(pre))
                        if (pre and gen and float(pre) > 0)
                        else None
                    )
    ...
    repro = [
        ...
        "",
        "```bash",
        f"python research/tr119/run_experiment.py --config {cfg_path} --device cuda",
        "```",
        "",
        "### 7.2 Key artifacts",
        "",

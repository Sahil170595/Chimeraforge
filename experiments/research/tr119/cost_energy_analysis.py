...
def analyze_cost_breakdown(cost_df: pd.DataFrame) -> dict[str, Any]:
    ...
    for backend in cost_df["backend"].unique():
        ...
        infra_mean = float(
            backend_data["infra_cost_usd_per_1m_tokens_on_demand"].mean()
        )
        energy_mean = float(backend_data["energy_cost_usd_per_1m_tokens"].mean())
        total_mean = float(
            backend_data["total_cost_usd_per_1m_tokens_on_demand"].mean()
        )
...
def analyze_roi_by_tier(
    ...
    for backend in cost_df["backend"].unique():
        ...
        if "total_cost_usd_per_1m_tokens_reserved" in backend_data.columns:
            reserved = float(
                backend_data["total_cost_usd_per_1m_tokens_reserved"].mean()
            )
        elif "total_cost_usd_per_1m_tokens_reserved_1yr" in backend_data.columns:
            reserved = float(
                backend_data["total_cost_usd_per_1m_tokens_reserved_1yr"].mean()
            )
        elif "total_cost_usd_per_1m_tokens_reserved_3yr" in backend_data.columns:
            reserved = float(
                backend_data["total_cost_usd_per_1m_tokens_reserved_3yr"].mean()
            )

        spot_savings = (
            compute_roi_savings(on_demand, spot, tokens_per_month, months)
            if spot is not None
            else None
        )
        reserved_savings = (
        ...
        roi[backend] = {
            ...
            "reserved_savings_pct": (
                reserved_savings["savings_pct"] if reserved_savings else None
            ),
            "reserved_savings_usd": (
                reserved_savings["savings_usd"] if reserved_savings else None
            ),
            "best_tier": best_tier,
            "best_cost": best_cost,
        }
...
def analyze_energy_efficiency(cost_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        cost_df.groupby("backend")["tokens_per_kwh"].mean().sort_values(ascending=False)
    )
...
def analyze_carbon_comparison(cost_df: pd.DataFrame) -> dict[str, Any]:
    ...
    carbon = (
        cost_df.groupby("backend")["carbon_gco2e_per_1m_tokens"].mean().sort_values()
    )
    return {
    ...
    for backend in cost_df["backend"].unique():
        ...
        if upfront_cost_usd > 0:
            ...
            for m in months:
                ...
                if (
                    cumulative >= upfront_cost_usd
                    and amort["break_even_months"] is None
                ):
...
def main() -> int:
    ...
    tokens_per_month = float(
        args.tokens_per_month or analysis_cfg.get("tokens_per_month", 1_000_000_000.0)
    )
    ...
    amortization = analyze_cost_amortization(
        cost_df, upfront_cost_usd, tokens_per_month
    )
    ...
    if prompt_tokens and generate_tokens:
        ...
        on_demand_rate = float(
            cfg.get("cloud_pricing", {}).get("on_demand_usd_per_hour", 1.0)
        )
        for backend in cost_df["backend"].unique():
            prefill = cost_df[
                (cost_df["backend"] == backend) & (cost_df["mode"] == "prefill")
            ]
            generate = cost_df[
                (cost_df["backend"] == backend) & (cost_df["mode"] == "generate")
            ]
            ...
            energy_kwh = (
                prefill_power * prefill_time_s + gen_power * gen_time_s
            ) / 3.6e6
    ...
    for backend, data in sorted(
        breakdown.items(), key=lambda x: x[1]["total_cost_usd_per_1m"]
    ):
        print(f"{backend}:")
        print(f"  Total: ${data['total_cost_usd_per_1m']:.4f}/1M tokens")
        print(
            f"  Infra: ${data['infra_cost_usd_per_1m']:.4f} ({data['infra_pct']:.1f}%)"
        )
        print(
            f"  Energy: ${data['energy_cost_usd_per_1m']:.4f} ({data['energy_pct']:.1f}%)"
        )
    ...
    print("\n=== Carbon Footprint ===")
    print(
        f"Lowest: {carbon['lowest_carbon_backend']} ({carbon['lowest_carbon_gco2e']:.1f} gCO2e/1M tokens)"
    )
    print(
        f"Highest: {carbon['highest_carbon_backend']} ({carbon['highest_carbon_gco2e']:.1f} gCO2e/1M tokens)"
    )

...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="TR119 analysis (latency + cost/energy)"
    )
    parser.add_argument(
        "--config",
    ...
    raw_files = sorted(
        raw_dir.glob("bench_tr119_*.jsonl"), key=lambda p: p.stat().st_mtime
    )
    ...
    for (backend, scenario, mode), group_all in df.groupby(
        ["backend", "scenario", "mode"]
    ):
        group_ok = df_ok[
            (df_ok["backend"] == backend)
            & (df_ok["scenario"] == scenario)
            & (df_ok["mode"] == mode)
        ]
        metrics = [
            m
            for m in group_ok.get("resource_metrics", []).tolist()
            if isinstance(m, dict)
        ]
        lat_vals = [float(v) for v in group_ok["run_mean_latency_ms"].dropna().tolist()]
        thr_vals = [
            float(v) for v in group_ok["run_mean_throughput_tok_s"].dropna().tolist()
        ]
        ...
        row: dict[str, Any] = {
            ...
            "cpu_utilization_mean_pct": _mean_metric(
                metrics, "cpu_utilization_mean_pct"
            ),
        }
    ...
    cpu_idle_watts = (
        float(cpu_power_cfg.get("idle_watts", 10.0)) if cpu_power_cfg else 0.0
    )
    cpu_max_watts = (
        float(cpu_power_cfg.get("max_watts", 65.0)) if cpu_power_cfg else 0.0
    )
    (float(cpu_power_cfg.get("default_utilization_pct", 0.0)) if cpu_power_cfg else 0.0)
    price_per_hour = float(cloud_cfg.get("on_demand_usd_per_hour", 1.0))
    spot_price_per_hour = float(
        cloud_cfg.get("spot_usd_per_hour", price_per_hour * 0.3)
    )
    ...
    for r in rows:
        ...
        cpu_power_w = (
            float(cpu_power_raw) if isinstance(cpu_power_raw, (int, float)) else None
        )
        cpu_util = float(r.get("cpu_utilization_mean_pct") or 0.0)
        cpu_power_est = None
        if cpu_power_cfg and cpu_power_w is None and cpu_util > 0:
            cpu_power_est = cpu_idle_watts + (cpu_util / 100.0) * max(
                0.0, cpu_max_watts - cpu_idle_watts
            )
        ...
        energy_j_per_1m = (
            compute_energy_from_power(power_w, seconds_per_1m) if power_w > 0 else 0.0
        )
        energy_j_per_token = (
            energy_j_per_1m / 1_000_000.0 if energy_j_per_1m > 0 else 0.0
        )
        ...
        # Carbon footprint
        carbon_gco2e_per_1m = compute_carbon_footprint(
            energy_kwh_per_1m, carbon_intensity
        )
        ...
        for tier_name, cost in tier_costs.items():
            ...
            tokens_per_dollar[tier_name] = (
                tok_per_hour / per_hour if per_hour > 0 else 0.0
            )

        # Efficiency metrics
        gpu_memory_mb = float(r.get("gpu_memory_mean_mb") or 0.0)
        memory_efficiency = (
            thr / gpu_memory_mb if gpu_memory_mb > 0 else 0.0
        )  # tokens/s per MB
        compute_efficiency = (
            thr / lat_mean_ms if lat_mean_ms > 0 else 0.0
        )  # tokens/s per ms
        ...
        row_cost: dict[str, Any] = {
            ...
            "cpu_power_mean_watts": (
                cpu_power_w if cpu_power_w and cpu_power_w > 0 else None
            ),
            ...
            "energy_cost_usd_per_1m_tokens": tier_costs["on_demand"][
                "energy_cost_usd_per_1m_tokens"
            ],
        ...
        for tier_name, cost in tier_costs.items():
            row_cost[f"infra_cost_usd_per_1m_tokens_{tier_name}"] = cost[
                "infra_cost_usd_per_1m_tokens"
            ]
            row_cost[f"total_cost_usd_per_1m_tokens_{tier_name}"] = cost[
                "total_cost_usd_per_1m_tokens"
            ]
            row_cost[f"cost_per_hour_usd_{tier_name}"] = cost_per_hour.get(tier_name)
            row_cost[f"tokens_per_dollar_{tier_name}"] = tokens_per_dollar.get(
                tier_name
            )
    ...
    cost_summary_path.write_text(
        json.dumps({"rows": cost_rows}, indent=2), encoding="utf-8"
    )

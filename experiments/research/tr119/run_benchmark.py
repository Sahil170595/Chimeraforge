...
from research.tr118.run_benchmark import (
    _compute_seq_len,
    _generate_uncached,
    _infer_ort,
    _infer_ort_logits,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="TR119 benchmark runner with cost/energy telemetry"
    )
    parser.add_argument(
        "--config",
    ...
    prompt_cfg_value = (
        ...
        or "research/tr117/configs/matrix_tier3.yaml"
    )
    ...
    seed = (
        ...
        else cfg.get("benchmark", {}).get("seed", None)
    )
    if seed is None:
    ...
    needs_onnx = any(b.startswith(("onnxruntime", "tensorrt")) for b in backends)
    ...
    if needs_onnx:
        ...
        export_path, _export_meta = _maybe_export_onnx(
            tr118_cfg, meta_path=export_meta_path
        )
        if export_path and export_path.exists() and needs_trt:
            engines, _trt_build_meta = _maybe_build_trt(
                tr118_cfg, export_path, meta_path=trt_build_meta_path
            )
    ...
    enable_resource = bool(
        cfg.get("telemetry", {}).get("enable_gpu", True)
        or cfg.get("telemetry", {}).get("enable_cpu", True)
    )
    sample_interval = float(
        cfg.get("telemetry", {}).get("sample_interval_s", 0.25) or 0.25
    )
        ...
        for backend_name, providers in [
            ...
            if backend_name in backends:
                try:
                    ...
                    session, input_type, used = _onnxruntime_session(
                        export_path, providers
                    )
    ...
    def get_torch_runner(
        device: str, compile_model: bool, batch_size: int, seq_len: int, model_name: str
    ) -> dict[str, Any] | None:
    ...
    def run_one_backend_scenario(
        backend: str, scenario: str, rep: int
    ) -> dict[str, Any]:
        _prompt_set, prompts, batch_size = _resolve_scenario(
                ...
                try:
                    if mode == "generate":
                        ...
                        if backend.startswith("onnxruntime"):
                            ...
                            total_ms, _ttft_ms, toks, _out_text, is_deg, reason = (
                                _generate_uncached(
                                    _infer_logits,
                                    tokenizer,
                                    prompt,

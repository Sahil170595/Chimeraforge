...
def analyze_cost_statistics(
    ...
    return {
        ...
        "anova": anova_results,
        "bootstrap_cis": {
            k: {"lower": v[0], "upper": v[1]} for k, v in bootstrap_cis.items()
        },
    }
...
def main() -> int:
    ...
    if args.cost_summary:
        ...
        results_dir = resolve_repo_path(_REPO_ROOT, "research/tr119/results")
        processed_dir = results_dir / "processed"
        cost_files = sorted(
            processed_dir.glob("cost_energy_summary*.json"),
    ...
    # Save report
    out_path = (
        resolve_repo_path(_REPO_ROOT, str(args.output))
        if args.output
        else cost_path.parent / "statistical_analysis.json"
    )
        ...
        if data.get("pairwise_comparisons"):
            ...
            for comp in data["pairwise_comparisons"]:
                if comp["significant"]:
                    ...
                    print(
                        f"  Difference: ${comp['difference']:.4f} ({comp['percent_change']:.1f}%)"
                    )
                    print(
                        f"  p-value: {comp['p_value']:.4f}, Cohen's d: {comp['effect_size_cohens_d']:.3f}"
                    )

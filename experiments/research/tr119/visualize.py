...
def main() -> int:
    ...
    for mode in modes:
        ...
        if "throughput_mean_tok_s" in df_mode.columns:
            thr = (
                df_mode.groupby("backend")["throughput_mean_tok_s"]
                .mean()
    ...
    if cost_json.exists():
        ...
        if rows:
            ...
            for mode in cost_modes:
                ...
                plt.title(
                    f"TR119 total cost per 1M tokens (on-demand) by backend ({mode})"
                )
                plt.xticks(rotation=45, ha="right")
                plt.tight_layout()
                ...
                tier_cols = [c for c in tier_labels if c in cost_mode.columns]
                if (
                    len(tier_cols) >= 2
                    and "total_cost_usd_per_1m_tokens_on_demand" in tier_cols
                ):
                    cost_by_tier = (
                        cost_mode.groupby("backend")[tier_cols]
                        .mean()
                        .sort_values("total_cost_usd_per_1m_tokens_on_demand")
                    )
                ...
                if "energy_kwh_per_1m_tokens" in cost_mode.columns:
                    ...
                    if not eff_df.empty:
                        eff_df["tokens_per_kwh"] = (
                            1_000_000.0 / eff_df["energy_kwh_per_1m_tokens"]
                        )
                        eff = (
                        ...
                        plt.title(
                            f"TR119 energy efficiency (tokens per kWh) by backend ({mode})"
                        )
                        plt.xticks(rotation=45, ha="right")
                        plt.tight_layout()
                ...
                if "carbon_gco2e_per_1m_tokens" in cost_mode.columns:
                    carbon = (
                        cost_mode.groupby("backend")["carbon_gco2e_per_1m_tokens"]
                        .mean()
                        .sort_values()
                    )
                    plt.figure(figsize=(10, 4))
                    ...
                    plt.title(
                        f"TR119 carbon footprint per 1M tokens by backend ({mode})"
                    )
                    plt.xticks(rotation=45, ha="right")
                    plt.tight_layout()
                    ...
                    for backend in cost_scatter.index:
                        plt.annotate(
                            ...
                            (
                                ...
                                cost_scatter.loc[
                                    backend, "total_cost_usd_per_1m_tokens_on_demand"
                                ],
                            ),
                            fontsize=8,
                        )

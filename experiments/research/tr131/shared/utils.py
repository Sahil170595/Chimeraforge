"""TR131 shared utilities — paths, constants, prompt generation."""
...
log = logging.getLogger("tr131.utils")

# ── Path anchors ──────────────────────────────────────────────────────
_DIR = Path(__file__).resolve().parent
_TR131 = _DIR.parent
_REPO = _TR131.parents[1]

TR131_RESULTS = _TR131 / "results"
TR131_CONFIG = _TR131 / "config.yaml"
...
NCU_PATH = Path(
    "C:/Program Files/NVIDIA Corporation/" "Nsight Compute 2025.3.1/ncu.bat"
)
...
# ── Seed paragraph for synthetic prompts (same as TR128-TR130) ────────
SEED_PARAGRAPH = (
...
def generate_prompts(
    ...
    return [generate_prompt(int(l)) for l in lengths]
...
def capture_environment() -> dict:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            env["ncu_version"] = lines[-1].strip() if lines else "unknown"
    except Exception:
        pass
    return env

"""Setup script for TR125 Phase 1: discover and pull Ollama model tags.

Pulls all 18 Ollama model tags required for the quantization decision matrix.
Each of the 3 base models needs 6 quant variants (Q2_K through Q8_0).

Usage:
    python research/tr125/phase1/setup_ollama.py [--dry-run] [--check-only]
    python research/tr125/phase1/setup_ollama.py --ollama-url http://localhost:11434
"""
...
logger = logging.getLogger("tr125.phase1.setup")

# All 18 required tags: 3 base models × 6 quant levels
...
def check_tag_available(tag: str, local_tags: set[str]) -> bool:
    ...
    if tag in local_tags:
        return True
    # Ollama sometimes appends ":latest" or version suffixes
    for local in local_tags:
        if local.startswith(tag) or tag.startswith(local.split(":")[0] + ":"):
            # Only match if the quant suffix matches too
            if tag in local or local == tag:
                return True
    return False
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Setup Ollama models for TR125 Phase 1"
    )
    ...
    if local:
        logger.debug("Local tags: %s", ", ".join(sorted(local)[:20]))
    ...
    print(f"TR125 {mode}: {ok} OK, {fail} MISSING of {len(results)} tags")
    print(f"{'='*70}")

    for r in results:
        status = "OK  " if r.available else "MISS"
        print(f"  [{status}] {r.tag:30s} {r.quant_level:8s} {r.message}")
    ...
    # Exit 0 even with missing tags — run.py handles partial availability
    return 0

...
def _fit_log_multivariate(
    *,
    ...
    X = np.column_stack(
        [np.ones_like(x_params_m), np.log(x_params_m), np.log(x_layers)]
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--include-warmup",
        action="store_true",
        help="Include warmup rows in summaries and fits.",
    )
    ap.add_argument(
        "--bootstrap",
        type=int,
        default=1000,
        help="Bootstrap reps for slope CIs (>=3 models required).",
    )
    ...
    if "params_millions_measured" in df_all.columns:
        ...
        df_all["params_millions_effective"] = measured.where(
            np.isfinite(measured) & (measured > 0), base
        )
    else:
        df_all["params_millions_effective"] = pd.to_numeric(
            df_all["params_millions"], errors="coerce"
        )
    ...
    if {"model_kind", "model"}.issubset(set(df_all.columns)):
        hf_models = sorted(
            {
                str(m)
                for m in df_all[df_all["model_kind"].astype(str) == "hf"][
                    "model"
                ].unique()
            }
        )
    ...
    df_all["prompt_tokens_compute"] = (bs * seq_len).where(
        np.isfinite(seq_len) & (seq_len > 0), np.nan
    )
    df_all["tokens_total_compute"] = pd.to_numeric(
        df_all.get("tokens_total"), errors="coerce"
    )
    ...
    df_all.loc[is_hf & is_prefill, "tokens_total_compute"] = df_all.loc[
        is_hf & is_prefill, "prompt_tokens_compute"
    ]
    df_all.loc[is_hf & is_e2e, "tokens_total_compute"] = df_all.loc[
        is_hf & is_e2e, "prompt_tokens_compute"
    ] + pd.to_numeric(df_all.loc[is_hf & is_e2e, "gen_tokens"], errors="coerce")
    df_all["tokens_per_s_compute"] = df_all["tokens_total_compute"] / (
        df_all["latency_ms"] / 1000.0
    )
    ...
    if "is_warmup" in df_all.columns and bool(df_all["is_warmup"].any()):
        ...
        eff = (
            ...
            .agg(
                warmup_n=("latency_ms", "count"),
                warmup_median_ms=("latency_ms", "median"),
            )
            .reset_index()
            .merge(
                meas.groupby(["backend", "model", "scenario", "mode"], dropna=False)
                .agg(
                    measured_n=("latency_ms", "count"),
                    measured_median_ms=("latency_ms", "median"),
                )
                .reset_index(),
        ...
        eff["warmup_to_measured_ratio"] = (
            eff["warmup_median_ms"] / eff["measured_median_ms"]
        )
        eff["warmup_minus_measured_ms"] = (
            eff["warmup_median_ms"] - eff["measured_median_ms"]
        )
        ...
        if {"backend_kind", "mode", "prefill_ms", "ollama_load_ms"}.issubset(
            set(df_all.columns)
        ):
            ow = warm[
                (warm["backend_kind"].astype(str) == "ollama")
                & (warm["mode"].astype(str) == "prefill")
            ].copy()
            om = meas[
                (meas["backend_kind"].astype(str) == "ollama")
                & (meas["mode"].astype(str) == "prefill")
            ].copy()
            if ow.shape[0] > 0 and om.shape[0] > 0:
                for d in (ow, om):
                    d["prefill_ms"] = pd.to_numeric(d["prefill_ms"], errors="coerce")
                    d["ollama_load_ms"] = pd.to_numeric(
                        d["ollama_load_ms"], errors="coerce"
                    )
                    d["prefill_total_ms_including_load"] = (
                        d["prefill_ms"] + d["ollama_load_ms"]
                    )
                ...
                bw = (
                    ...
                    .agg(
                        ...
                        warmup_prefill_total_median_ms=(
                            "prefill_total_ms_including_load",
                            "median",
                        ),
                ...
                bm = (
                    ...
                    .agg(
                        ...
                        measured_prefill_total_median_ms=(
                            "prefill_total_ms_including_load",
                            "median",
                        ),
                ...
                bd["warmup_ratio_infer_only"] = (
                    bd["warmup_prefill_infer_median_ms"]
                    / bd["measured_prefill_infer_median_ms"]
                )
                bd["warmup_ratio_total_including_load"] = (
                    bd["warmup_prefill_total_median_ms"]
                    / bd["measured_prefill_total_median_ms"]
                )
    ...
    if {"backend_kind", "gen_tokens_raw", "gen_tokens_target", "mode"}.issubset(
        set(df_all.columns)
    ):
        oll = df_all[
            ...
            by = (
                ...
                .agg(
                    ...
                    done_reason_mode=(
                        "done_reason",
                        lambda s: (
                            s.mode(dropna=True).iloc[0] if s.notna().any() else None
                        ),
                    ),
                )
                .reset_index()
    ...
    if {
        ...
        od["kv_decode_ms_equiv"] = pd.to_numeric(
            od["kv_decode_ms_equiv"], errors="coerce"
        )
        od["gen_tokens_raw"] = pd.to_numeric(od["gen_tokens_raw"], errors="coerce")
        od["gen_tokens_target"] = pd.to_numeric(
            od["gen_tokens_target"], errors="coerce"
        )
        od = od[
        ...
        if od.shape[0] > 0:
            ...
            for model, g in od.groupby("model", dropna=False):
                ...
                fit = _fit_linear(
                    g["gen_tokens_raw"].to_numpy(), g["kv_decode_ms"].to_numpy()
                )
                ...
                prop_equiv = pd.to_numeric(
                    g["kv_decode_ms_equiv"], errors="coerce"
                ...
                lin_rows.append(
                    {
                        ...
                        "gen_tokens_raw_min": float(
                            np.nanmin(g["gen_tokens_raw"].to_numpy())
                        ),
                        "gen_tokens_raw_median": float(
                            np.nanmedian(g["gen_tokens_raw"].to_numpy())
                        ),
                        "gen_tokens_raw_max": float(
                            np.nanmax(g["gen_tokens_raw"].to_numpy())
                        ),
                    }
                )
                err_rows.append(
                    {
                        ...
                        "prop_minus_affine_ms_p95": float(
                            np.nanquantile(prop_err, 0.95)
                        ),
    ...
    if {"cuda_event_ms", "latency_ms"}.issubset(set(df_all.columns)):
        ...
        if c.shape[0] > 0:
            ...
            c.groupby(
                ["backend", "backend_kind", "mode", "is_warmup"], dropna=False
            ).agg(
                n=("rel_gap", "count"),
                rel_gap_median=("rel_gap", "median"),
                rel_gap_p95=(
                    "rel_gap",
                    lambda s: float(np.quantile(s.to_numpy(), 0.95)),
                ),
    ...
    agg = (
        ...
        .agg(
            ...
            latency_p95_ms=(
                "latency_ms",
                lambda s: float(np.quantile(s.to_numpy(dtype=float), 0.95)),
            ),
            latency_p99_ms=(
                "latency_ms",
                lambda s: float(np.quantile(s.to_numpy(dtype=float), 0.99)),
            ),
    ...
    by_model_mode = (
        ...
        .agg(
            ...
            latency_geomean_ms=(
                "latency_median_ms",
                lambda s: _geomean(s.to_numpy(dtype=float)),
            ),
            ...
            tokens_per_s_compute_geomean=(
                "tokens_per_s_compute_median",
                lambda s: _geomean(s.to_numpy(dtype=float)),
            ),
            ...
            params_millions_measured_median=(
                "params_millions_measured_median",
                "median",
            ),
        )
    ...
    by_model_mode.to_csv(
        analysis_dir / "summary_by_model_backend_mode_agg.csv", index=False
    )
    ...
    if not hf_arch.empty:
        ...
        mv["params_millions_effective"] = pd.to_numeric(
            mv["params_millions_effective"], errors="coerce"
        )
        mv["latency_geomean_ms"] = pd.to_numeric(
            mv["latency_geomean_ms"], errors="coerce"
        )
        ...
        for (backend, batch_size, mode), g in mv.groupby(
            ["backend", "batch_size", "mode"], dropna=False
        ):
            ...
            if boot_betas:
                row.update(
                    {
                        "beta_ci_low": float(
                            np.quantile(np.asarray(boot_betas), 0.025)
                        ),
                        "beta_ci_high": float(
                            np.quantile(np.asarray(boot_betas), 0.975)
                        ),
                        "gamma_ci_low": float(
                            np.quantile(np.asarray(boot_gammas), 0.025)
                        ),
                        "gamma_ci_high": float(
                            np.quantile(np.asarray(boot_gammas), 0.975)
                        ),
                        "r2_ci_low": float(np.quantile(np.asarray(boot_r2), 0.025)),
                        "r2_ci_high": float(np.quantile(np.asarray(boot_r2), 0.975)),
                        "boot_n_used": len(boot_betas),
                        "boot_n_requested": int(n_boot),
                    }
                )
            mv_rows.append(row)
    ...
    if not hf_arch.empty and spearmanr is not None:
        hm = by_model_mode[by_model_mode["backend_kind"].astype(str) == "hf"].merge(
            hf_arch, on="model", how="left"
        )
        corr_rows: list[dict[str, Any]] = []
        for (backend, batch_size, mode), g in hm.groupby(
            ["backend", "batch_size", "mode"], dropna=False
        ):
    ...
    for (backend, backend_kind, mode, scenario), g in agg.groupby(
        ...
        ci = _bootstrap_power_law_ci(
            x=x, y=y, n_boot=int(args.bootstrap), seed=int(args.seed)
        )
        fit_rows.append(
            {
                "backend": backend,
                ...
                "batch_size": (
                    float(g["batch_size"].iloc[0])
                    if "batch_size" in g.columns and g.shape[0]
                    else float("nan")
                ),
                "mode": mode,
                "scenario": scenario,
    ...
    if {"backend_kind", "gen_tokens_raw", "gen_tokens_target", "mode"}.issubset(
        ...
        if fit_len_rows:
            pd.DataFrame(fit_len_rows).sort_values(["backend", "mode"]).to_csv(
                out_len, index=False
            )
        else:
            ...
            overhead_rows.append(
                {
                    ...
                    "n_models": int(g.shape[0]),
                    **lin,
                    "overhead_fraction_at_min_model": (
                        float(
                            lin["a_ms"]
                            / (lin["a_ms"] + lin["b_ms_per_million_params"] * x_min)
                        )
                        if (lin["a_ms"] + lin["b_ms_per_million_params"] * x_min) > 0
                        else float("nan")
                    ),
                    "overhead_fraction_at_max_model": (
                        float(
                            lin["a_ms"]
                            / (lin["a_ms"] + lin["b_ms_per_million_params"] * x_max)
                        )
    ...
    try:
        ...
        for (backend, _backend_kind, mode, scenario), g in agg.groupby(
            ["backend", "backend_kind", "mode", "scenario"], dropna=False
        ):
            if g.shape[0] < 2:
                continue
            ...
            if fit.get("ok"):
                xline = np.logspace(
                    np.log10(float(np.min(xs))), np.log10(float(np.max(xs))), 100
                )
                yline = (10 ** float(fit["log10_intercept"])) * (
                    xline ** float(fit["slope"])
                )
        ...
        for (backend, _backend_kind, batch_size, mode), g in by_model_mode.groupby(
            ["backend", "backend_kind", "batch_size", "mode"], dropna=False
        ):
            if g.shape[0] < 2:
                continue
        ...
        if early_csv.exists():
            ...
            if {
                "gen_tokens_raw_median",
                "params_millions_effective_median",
                "model",
            }.issubset(set(e.columns)) and e.shape[0] > 0:

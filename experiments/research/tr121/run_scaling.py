...
def _load_hf(model_path: str, device: str, dtype: str):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_path, local_files_only=True, dtype=_dtype_from_str(dtype)
    )
    model.eval()
...
def _ollama_generate(
    url: str, model: str, prompt: str, options: dict[str, Any], timeout_s: float
) -> dict[str, Any]:
    payload = {
        "model": model,
        "prompt": prompt,
...
def _measure_ollama(
    *,
    ...
    kv_decode_ms_equiv = (
        (kv_ms_per_token * float(gen_tokens)) if eval_tokens > 0 else eval_ms
    )
...
def main() -> int:
    ...
    ap.add_argument("--config", default="research/tr121/configs/scaling.yaml")
    ...
    run_id = _now_run_id()
    out_dir = (
        Path(args.out_dir) if args.out_dir else Path("research/tr121/results") / run_id
    )
    ...
    repetitions = (
        ...
        else int(cfg.get("repetitions") or 1)
    )
    warmups = (
        int(args.warmup_repetitions)
        if args.warmup_repetitions is not None
        else int(cfg.get("warmup_repetitions") or 0)
    )
    gen_tokens = (
        int(args.gen_tokens)
        if args.gen_tokens is not None
        else int(cfg.get("gen_tokens") or 64)
    )
    ...
    allow_models = (
        {s.strip() for s in str(args.models).split(",")} if args.models else None
    )
    allow_backends = (
        {s.strip() for s in str(args.backends).split(",")} if args.backends else None
    )
    allow_scenarios = (
        {s.strip() for s in str(args.scenarios).split(",")} if args.scenarios else None
    )
    ...
    resolved = {
        "scenarios": [
            {"name": str(s.get("name")), "prompt": str(s.get("prompt"))}
            for s in scenarios
        ],
        ...
        "filters": {
            "models_allowlist": (
                sorted(allow_models) if allow_models is not None else None
            ),
            "backends_allowlist": (
                sorted(allow_backends) if allow_backends is not None else None
            ),
            "scenarios_allowlist": (
                sorted(allow_scenarios) if allow_scenarios is not None else None
            ),
        },
    }
    ...
    def project_mode(meas: dict[str, Any], mode: str) -> dict[str, Any]:
        ...
        tok_s = (
            (tokens_total / (latency_ms / 1000.0)) if latency_ms > 0 else float("nan")
        )
        return {
            "mode": mode,
    ...
    for backend in backends:
        ...
        try:
            tags = _ollama_tags(
                str(backend.url), timeout_s=float(args.ollama_timeout_s)
            )
    ...
    def _get_or_load_hf(
        model_name: str, device: str, dtype: str
        ...
        try:
            ...
            hf_params_millions[key] = (
                float(sum(p.numel() for p in hf_model.parameters())) / 1e6
            )
        except Exception:
        ...
        for task_idx, (scenario, model, backend) in enumerate(tasks):
            ...
            for warmup_idx in range(warmups):
                ...
                for mode in ("prefill", "kv_decode", "e2e_kv"):
                    projected = (
                        project_mode(meas, mode) if meas.get("status") == "ok" else {}
                    )
                    rec = {
                        **base_meta,
    ...
    _write_csv(
        out_dir / "hf_load_ms.csv",
        [
            {"model": k[0], "device": k[1], "dtype": k[2], "hf_load_ms": v}
            for k, v in hf_load_ms.items()
        ],
    )
    _write_csv(
    ...
    manifest["results"] = {
        ...
        "resolved_model_params_csv": _relpath_safe(
            out_dir / "resolved_model_params.csv", _repo_root()
        ),
    }

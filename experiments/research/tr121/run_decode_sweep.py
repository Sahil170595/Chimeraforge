...
Outputs:
  research/tr121/results/tr121_decode_sweep/<RUN_ID>/
    gen_8/<subrun>/
    gen_32/<subrun>/
    ...
  plus a combined CSV in:
    research/tr121/results/tr121_decode_sweep/<RUN_ID>/decode_sweep_summary.csv
"""
...
def _decode_fraction_tables(
    *, run_dir: Path, gen_tokens_target: int
    ...
    merged["decode_fraction"] = (
        merged["kv_decode_latency_ms"] / merged["e2e_kv_latency_ms"]
    )
...
def main() -> int:
    ...
    ap.add_argument("--config", default="research/tr121/configs/decode_sweep.yaml")
    ...
    out_root = (
        Path(args.out_root)
        if args.out_root
        else Path("research/tr121/results/tr121_decode_sweep") / run_id
    )
    ...
    for gen in gen_tokens_list:
        ...
        cmd = [
            ...
            str(run_scaling),
            "--config",
            str(Path("research/tr121/configs/scaling.yaml")),
            "--out-dir",
            str(out_dir),
            "--gen-tokens",
            str(gen),
        ...
        sel = fits[
            (fits["scenario"] == "__all__")
            & (fits["aggregation"] == "geomean_across_scenarios")
        ].copy()
        sel["gen_tokens_target"] = int(gen)
        rows.extend(sel.to_dict(orient="records"))

        per_model, summary = _decode_fraction_tables(
            run_dir=out_dir, gen_tokens_target=int(gen)
        )
    ...
    if frac_summary:
        ...
        df_summary = (
            df_summary.groupby(
                ["backend", "backend_kind", "gen_tokens_target"], dropna=False
            )
            .agg(

...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="TR120 compile paradox analysis (TR117 artifacts)"
    )
    parser.add_argument(
        "--metrics-csv",
    ...
    parser.add_argument(
        "--out-dir",
        default="research/tr120/results/tr120_compile_paradox",
    ...
    for (backend, quant, scenario), group in df.groupby(
        ["backend", "quantization", "scenario"]
    ):
        if backend not in backends:
            continue
        vals = group["latency_ms"].dropna().to_numpy(dtype=float)
    ...
    df_overall = (
        pd.DataFrame(rows_overall).sort_values("mean_ms").reset_index(drop=True)
    )
    df_quant = (
        pd.DataFrame(rows_quant)
        .sort_values(["quantization", "mean_ms"])
    ...
    df_scenario_quant.to_csv(
        processed_dir / "summary_by_scenario_quantization.csv", index=False
    )
    ...
    if len(backends) == 2:
        ...
        def compare(sub_df: pd.DataFrame) -> dict[str, Any]:
            a = (
                sub_df[sub_df["backend"] == a_name]["latency_ms"]
                .dropna()
                .to_numpy(dtype=float)
            )
            b = (
                sub_df[sub_df["backend"] == b_name]["latency_ms"]
                .dropna()
                .to_numpy(dtype=float)
            )
            sa = _summary(a)
        ...
        for (quant, scenario), group in df.groupby(["quantization", "scenario"]):
            comparisons["by_scenario_quantization"].setdefault(str(quant), {})[
                str(scenario)
            ] = compare(group)
    ...
    backend_latencies_excl_first: dict[str, list[np.ndarray]] = {
        b: [] for b in backends
    }
        ...
        by_file_rows.append(
            {
                ...
                "first_over_2x_rest_median": bool(
                    np.isfinite(ratio_first_over_median_rest)
                    and ratio_first_over_median_rest > 2.0
                ),
                "first_over_3x_rest_median": bool(
                    np.isfinite(ratio_first_over_median_rest)
                    and ratio_first_over_median_rest > 3.0
                ),
    ...
    if not df_by_file.empty:
        df_by_file = df_by_file.sort_values(
            ["scenario", "quantization", "backend"]
        ).reset_index(drop=True)
        ...
        for backend in backends:
            lat_all = (
                np.concatenate(backend_latencies[backend])
                if backend_latencies[backend]
                else np.array([])
            )
            lat_excl = (
            ...
            frac_first_over_2x = float(
                df_by_file[df_by_file["backend"] == backend][
                    "first_over_2x_rest_median"
                ].mean()
            )
            frac_first_over_3x = float(
                df_by_file[df_by_file["backend"] == backend][
                    "first_over_3x_rest_median"
                ].mean()
            )
        ...
        df_cold_scenario.to_csv(
            processed_dir / "cold_start_effect_by_scenario_backend.csv", index=False
        )
    ...
    def plot_cdf(df_in: pd.DataFrame, title: str, path: Path) -> None:
        ...
        for backend in backends:
            vals = (
                df_in[df_in["backend"] == backend]["latency_ms"]
                .dropna()
                .to_numpy(dtype=float)
            )
            if vals.size == 0:
    ...
    plot_cdf(
        df,
        "TR120.A: Latency CDF (overall; TR117 tier3)",
        plots_dir / "cdf_latency_overall.png",
    )
    ...
    if not df_by_file.empty:
        out_json["cold_start"] = {
            ...
            "paths": {
                "by_file_csv": str(
                    (processed_dir / "cold_start_by_file.csv").as_posix()
                ),
                "overall_csv": str(
                    (processed_dir / "cold_start_effect_overall.csv").as_posix()
                ),
                "by_scenario_backend_csv": str(
                    (
                        processed_dir / "cold_start_effect_by_scenario_backend.csv"
                    ).as_posix()
                ),
            },
        }

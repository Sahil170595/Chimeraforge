...
Outputs:
  research/tr120/results/tr120_root_cause/<run_id>/<mode>/
    - runs/.../run_*.json
    - metrics.csv
    - manifest.json
"""
...
def _dynamo_explain_summary(model, *, call_kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(ops, list):
            ...
            for g in ops:
                try:
                    ops_summary.append(len(g))  # type: ignore[arg-type]
                except Exception:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="TR120 root-cause runner (explicit eager vs torch.compile)."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("research/tr120/configs/root_cause.yaml"),
        help="Runner config yaml.",
    )
    parser.add_argument(
        "--out-root",
        type=Path,
        default=Path("research/tr120/results/tr120_root_cause"),
    ...
    if not scenarios:
        raise SystemExit(
            "No scenarios selected; check config.scenarios against tier3_config.scenarios[].name"
        )
    ...
    try:
        ...
        prompts = [
            str(p) for p in tier3.get("prompt_sets", {}).get(prompt_set_name, [])
        ]
        if prompts:
            tokenization_cfg = dict(cfg.get("tokenization", {}) or {})
            tokenized = _tokenize_prompts(
                ...
                device,
                pad_to_max_length=bool(
                    tokenization_cfg.get("pad_to_max_length", False)
                ),
                pad_to_multiple_of=(
                    int(tokenization_cfg["pad_to_multiple_of"])
                    if tokenization_cfg.get("pad_to_multiple_of")
                    else None
                ),
                max_length=(
                    int(tokenization_cfg["max_length"])
                    if tokenization_cfg.get("pad_to_max_length")
                    and tokenization_cfg.get("max_length")
                    else None
                ),
            )
            if tokenized:
    ...
    for mode in modes:
        ...
        for scenario in scenarios:
            ...
            if not prompts:
                raise SystemExit(
                    f"prompt_set '{prompt_set_name}' missing/empty in {tier3_path}"
                )

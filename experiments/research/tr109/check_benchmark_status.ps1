...
if (Test-Path rust_benchmark_results) {
    ...
    if (Test-Path rust_benchmark_results\benchmark_results.json) {
        Write-Host ""
        Write-Host "✓ Benchmark complete! Run: python analyze_rust_results.py" -ForegroundColor Green
    } else {
        Write-Host ""
        Write-Host "⏳ Benchmark still running..." -ForegroundColor Yellow
    }
} else {
    Write-Host "⏳ Benchmark not started or no results yet..." -ForegroundColor Yellow
}

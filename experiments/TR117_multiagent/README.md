...
## Artifacts & Scripts
- `experiments/TR117_multiagent/PRD.md` - plan and success criteria.
- Scripts: `experiments/TR117_multiagent/scripts/`
...
- Results root: `experiments/TR117_multiagent/results/` (pre-created with `.gitkeep`).
...
```
python experiments/TR117_multiagent/scripts/profiler_agent.py \
  --model gemma3:latest \
  --runs 1 \
  --scenario chimera_homo \
  --output-dir experiments/TR117_multiagent/results/phase1/default
```
...
python experiments/TR117_multiagent/scripts/profiler_agent.py \
  ...
  --output-dir experiments/TR117_multiagent/results/phase1/buf64k
```
3) Throttle test (60 tok/s):
```
python experiments/TR117_multiagent/scripts/throttled_agent.py \
  --model gemma3:latest \
  --runs 1 \
  --throttle-rate 60 \
  --output-dir experiments/TR117_multiagent/results/phase3/60tps
```
4) GPU logging wrapper (replace CMD with your dual-agent run):
```
python experiments/TR117_multiagent/scripts/dmon_wrapper.py \
  --output-dir experiments/TR117_multiagent/results/phase2/gemma \
  -- your_benchmark_command_here
```
5) Tokenizer bench (local-only; add --allow-download if needed):
```
python experiments/TR117_multiagent/scripts/bench_tokenizer.py \
  --bytes 1048576 \
  --iterations 3 \
  --output-dir experiments/TR117_multiagent/results/phase2/tokenizers
```

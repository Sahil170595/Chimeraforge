...
- **Compiled decode crash is patchable from PyTorch internals (CORRECTED 2026-06-05):** v3 prototype experiments showed that the "never free" approach -- disabling `_free_And_Remove_DeleterFn` + `check_memory_pool` + the assertion -- still crashes, and the original wording concluded the fix "must come from the model/cache layer, not PyTorch internals." That conclusion was wrong: the *opposite* approach works. PR [#184102](https://github.com/pytorch/pytorch/pull/184102) (jansel, Fixes #175557) preserves/clones the cudagraph-pool-aliased inputs *before* the dealloc frees them, and patches the crash entirely in PyTorch internals -- validated on a real gpt2 decode (SS10.9). The architectural framing held only for the never-free direction this report tested. Issue [pytorch/pytorch#175557](https://github.com/pytorch/pytorch/issues/175557) filed; assertion fix [pytorch/pytorch#175562](https://github.com/pytorch/pytorch/pull/175562) **landed** in main; decode fix #184102 under maintainer review.
...
- **StaticCache enables compiled decode but 5.8× slower:** Replacing `DynamicCache` with `StaticCache` + `torch.compile(model.forward, mode="default")` produces the first successful compiled decode in this research program. But without CUDA graph replay, compilation overhead per decode step exceeds kernel optimization benefit (3,588 ms vs 622 ms eager). `reduce-overhead` + StaticCache still crashes. The StaticCache model-layer path is a dead end on stock PyTorch 2.10 / transformers 5.2.0. (CORRECTED 2026-06-05: this dead end is specific to the model-layer/StaticCache path. The PyTorch-internals input-preservation fix #184102 runs `reduce-overhead` compiled decode with a growing `DynamicCache` to completion on a real gpt2 -- see SS10.9.)
...
> **Update / Resolution (2026-06-05):** The assertion fix [PR #175562](https://github.com/pytorch/pytorch/pull/175562) landed in PyTorch main (squash commit `be90a1495310`, 2026-06-04). More importantly, this report's central forward-looking conclusion -- that the compiled-decode crash is architectural and "must come from the model/cache layer, not PyTorch internals" -- is **corrected**: that held only for the "never free" sledgehammer and StaticCache paths this report tested. The opposite approach, preserving/cloning cudagraph-pool-aliased inputs before dealloc frees them, fixes the crash entirely from PyTorch internals. [PR #184102](https://github.com/pytorch/pytorch/pull/184102) (jansel, Fixes #175557) does this; we validated it on a real gpt2 `reduce-overhead` decode (128 tokens, growing `DynamicCache`) and 7 synthetic topologies. One synthetic multi-partition gap was found and reported upstream. #184102 is under maintainer review, not merged. All measurement data below is unchanged; see the new **SS10.9** for details.
...
4. **torch.compile is prefill-only in practice on stock PyTorch (all modes):** Both `reduce-overhead` and `mode="default"` crash on autoregressive decode (100% crash rate). PyTorch 2.8's Inductor uses CUDA graph trees internally regardless of mode. Even when compiled decode works (64 tokens, Phase 2), it provides no speedup (+2.2%). On a stock PyTorch, production must use eager or Ollama for decode. (Update 2026-06-05: a PyTorch carrying input-preservation fix #184102, in review, runs `reduce-overhead` compiled decode to completion on a real gpt2 -- SS10.9.)
...
12. **Bug persists on stock PyTorch 2.10:** Full Phase 3 rerun (4,522 measurements) on NGC 26.01 (PyTorch 2.10.0a0, CUDA 13.1, Triton 3.6.0) with PR #175562 pre-applied: compiled prefill works (-42.4%), compiled decode crashes 100%. The `dealloc_current_path_weakrefs` crash is not version-specific (SS10.6). (CORRECTED 2026-06-05: previously called "architectural"; it is fixable from PyTorch internals via input-preservation #184102, validated on real decode -- SS10.9.)
...
- **Decode recommendation (for stock PyTorch):** On a stock PyTorch, the "use Ollama for decode" recommendation is supported by five lines of evidence: (1) `reduce-overhead` crashes on decode at 128 tokens, (2) `mode="default"` also crashes (PyTorch 2.8/2.10 Inductor uses CUDA graph trees internally), (3) even when compiled decode works (64 tokens, Phase 2), it provides no speedup (+2.2%), (4) the 3-patch "never free" prototype on `cudagraph_trees.py` still crashed, and (5) StaticCache + `mode="default"` enables decode but is 5.8× *slower* than eager. CORRECTED 2026-06-05: the original "patching cudagraph_trees.py confirmed the crash is not patchable" and "every viable compiled decode path has been exhausted" claims were wrong -- the never-free direction fails, but the input-preservation fix #184102 patches it in PyTorch internals (validated on real decode, SS10.9). The Ollama recommendation stands for a stock PyTorch; compiled decode becomes viable on a PyTorch carrying #184102 (in review).
...
| 14 | ~~Crash is architectural, not patchable in PyTorch internals~~ -> Crash IS patchable from PyTorch internals via input-preservation (CORRECTED 2026-06-05) | v3's "never free" prototype (patch `dealloc_current_path_weakrefs()` + `check_memory_pool` + assertion) still crashes via `get_non_cudagraph_inps` (SS10.7) -- so that direction fails. But PR [#184102](https://github.com/pytorch/pytorch/pull/184102) (preserve/clone pool-aliased inputs before dealloc) fixes it entirely in PyTorch internals, validated on real gpt2 decode + 7 topologies (SS10.9). Assertion fix PR [#175562](https://github.com/pytorch/pytorch/pull/175562) landed in main. | **Corrected: patchable from PyTorch internals (#184102, in review)** |
...
**Answer:** On a stock PyTorch, only for prefill. Phase 3 (SS10) shows 100% crash rate on autoregressive decode with both `reduce-overhead` and `mode="default"` — PyTorch 2.8's Inductor uses CUDA graph trees internally regardless of mode setting (SS10.5). Use eager mode for decode, or use Ollama. (Update 2026-06-05: the crash is fixable from PyTorch internals; the input-preservation fix #184102, in review, runs compiled `reduce-overhead` decode to completion on a real gpt2 -- SS10.9.)
...
**Practical implication:** Compiled autoregressive decode is **not viable** with any torch.compile mode on **stock** PyTorch 2.8 (and 2.10; see SS10.6). For production systems on a stock PyTorch that need decode:
...
> **Update (2026-06-05):** "not viable with any torch.compile mode" was true for every PyTorch we measured, but it is no longer true in general. A PyTorch carrying PR #184102 (input-preservation fix, in review) runs this exact compiled `reduce-overhead` decode to completion on a real gpt2 with a growing cache -- validated. See SS10.9.
...
**Implication:** On a stock PyTorch 2.8 there is no torch.compile mode that enables compiled autoregressive decode. This closes the gap identified in v1's limitation L2 — on the PyTorch versions we measured, the answer is "no, compiled decode is not viable" rather than "untested." We verified the bug persists on stock PyTorch 2.10 (NGC 26.01). The assertion fix ([pytorch/pytorch#175562](https://github.com/pytorch/pytorch/pull/175562)) has since **landed** in main. (Update 2026-06-05: the separate decode fix #184102, in review, removes this limitation on a patched PyTorch -- compiled `reduce-overhead` decode then runs to completion; see SS10.9. On a stock PyTorch, compilation remains exclusively a prefill optimization.)
...
**Implication:** The compiled decode crash persists across stock PyTorch versions (2.8 and 2.10), so it is not a version-specific regression. SS10.7 prototypes the "never free" direction in `cudagraph_trees.py`, which fails. (CORRECTED 2026-06-05: the original wording -- "stems from an architectural incompatibility ... the root cause is at the model/cache layer, not in cudagraph_trees.py" -- was wrong. The opposite, input-preservation direction does fix it in `cudagraph_trees.py` from PyTorch internals: PR #184102, validated on real decode, SS10.9.)
...
- Under the "never free" patches tested here, these are incompatible. (CORRECTED 2026-06-05: "regardless of any `cudagraph_trees.py` patch" was too strong -- a different `cudagraph_trees.py` patch, input-preservation #184102, does reconcile them; see SS10.9.)

**Fix paths considered here (model/cache layer):**
...
This analysis supersedes the v2 characterization of the crash as a "bug" — it is more accurately described as an **architectural limitation** of CUDA graphs when combined with dynamic tensor operations. The upstream issue ([pytorch/pytorch#175557](https://github.com/pytorch/pytorch/issues/175557)) remains valid, and the assertion fix (PR #175562) is a genuine bug fix.

> **CORRECTION (2026-06-05):** The claim that "compiled decode requires model-level changes, not just PyTorch patches" -- and the original heading "Viable fix paths (all at the model/cache layer, not in PyTorch internals)" -- was wrong, and was wrong because it generalized from the wrong fix direction. The three patches above are a "never free" sledgehammer (refuse to free path weakrefs); that direction fails, as shown. The *opposite* direction -- preserve/clone the cudagraph-pool-aliased inputs *before* the dealloc frees them -- does patch the crash entirely from PyTorch internals. PR #184102 (jansel, Fixes #175557) implements this; we validated it on a real gpt2 decode. So #175557 is patchable in PyTorch internals, the model/cache layer is not the only path, and the four paths above are alternatives, not the only options. Full detail in SS10.9.

### 10.8 StaticCache Experiment: Model-Layer Fix Attempt (NEW in v3)

Having (at the time) concluded the crash was architectural (SS10.7 -- since corrected; see SS10.9), we tested the model-layer fix path: replacing `DynamicCache` with `StaticCache` (pre-allocated, no `torch.cat`) and compiling `model.forward` instead of the full model object.
...
> **Superseded in part by SS10.9 (2026-06-05).** The "fix must come from the model/cache layer, not PyTorch internals" conclusion above was correct only for the approaches tested in v3 (StaticCache and the three-patch "never free" sledgehammer). A PyTorch-internals fix that takes the *opposite* approach -- preserving the cudagraph-pool-aliased inputs before they are freed -- does patch the crash. See SS10.9.

### 10.9 Update / Resolution (2026-06-05): Decode Crash Is Fixable From PyTorch Internals

This section was added after upstream resolution. It corrects the forward-looking conclusions of SS10.6-SS10.8; **all measurement data above is unchanged.**

**1. The assertion fix (PR #175562) landed in PyTorch main.** Squash commit `be90a1495310` (2026-06-04), committed by the PyTorch MergeBot. It replaces the hard `assert len(node.tensor_weakrefs) == len(node.stack_traces)` in `dealloc_current_path_weakrefs` with a warning (defensive consistency with the sibling `outputs_weakrefs` handling). This is a small cleanup; as SS10.7 already established, it does **not** fix the decode crash. (The GitHub PR page shows "Closed" rather than a green "Merged" badge because the mergebot lands by pushing the squashed commit directly to main; the commit is in main.) URL: https://github.com/pytorch/pytorch/pull/175562

**2. CORRECTION: the decode crash IS patchable from PyTorch internals.** SS10.7 concluded the crash was "architectural, not patchable in cudagraph_trees.py" and that "the fix must come from the model/cache layer, not PyTorch internals." That conclusion holds **only for the approaches this report tested** -- the v3 three-patch "never free" sledgehammer (disable `_free_And_Remove_DeleterFn` + `check_memory_pool` + the assertion) and StaticCache. The correct fix takes the *opposite* approach: instead of refusing to free, it **preserves/clones the cudagraph-pool-aliased inputs before the dealloc frees them.** PR #184102 (jansel, "Fix cudagraph handoff across compiled calls", Fixes #175557) implements this via `_preserve_inputs_that_alias_current_path`. So the honest correction is: input-preservation patches the crash from PyTorch internals; the never-free approach does not. URL: https://github.com/pytorch/pytorch/pull/184102

**3. Validated on a real decode (not just synthetic).** We hand-applied #184102's diff (pure-Python, no rebuild) to two installed torch builds -- NGC pytorch:26.01 (torch 2.10.0a0, triton 3.6.0) and pip nightly torch 2.12.0.dev20260408 (triton 3.7.0, = current main) -- and ran a faithful version of this report's `_kv_decode` workload: stock `gpt2`, `mode="reduce-overhead"`, `past_key_values` fed back each step, 128 tokens.
   - **Baseline:** crashes at the first decode step with the exact TR126 error ("accessing tensor output of CUDAGraphs that has been overwritten by a subsequent run").
   - **With #184102:** completes all 128 tokens; cudagraphs stay active (the dynamic-shape recording warning fires, so it is not an eager fallback).
   This contradicts SS10.1/SS10.6's "compiled decode is not viable with any torch.compile mode" **for the patched case** -- on stock PyTorch the crash stands; on a PyTorch carrying #184102, compiled decode with a growing cache works. The fix also held across 7 synthetic feedback topologies (simple, no-mark, strided, cross-call, graph-break, graph_partition x2); baseline crashes all, #184102 fixes all.

**4. One coverage gap found and reported (synthetic).** When a compiled function splits into multiple cudagraph partitions and a top-level input consumed only by a *later* partition is fed back from the pool, #184102 still crashes ("overwritten"). Mechanism: each partition is cudagraphified independently and dispatched through its own `_run` with only its input slice; `_preserve` clones only the current partition's inputs, while `dealloc` frees the whole path -- so the later-partition input is freed before it can be cloned (confirmed by instrumenting `_preserve`: every call sees `n_inputs=1`, never the full arg list). Surgically isolated by feeding back the first-partition input (PASS) vs the later-partition input (CRASH), varying only the partition index. Reproduces identically on torch 2.10 and 2.12 nightly. **Caveat:** the partition split was forced synthetically via a DeviceCopy boundary (`.to('cpu').to('cuda')`); no real model has been shown to hit this topology (the real gpt2 decode does not), so practical blast radius is unproven and likely small. vmoens' alternative PR #176295 catches this case by raising a clear "please .clone()" error for partitioned calls. #184102 is not broken; this is one incompleteness.

**5. Status: #184102 is under active maintainer review, not merged (as of 2026-06-05).** eellison (core cudagraph_trees author) reviewed it and noted it should probably be opt-in (potentially a lot of copies) and that it needs to interact with cached outputs. So the upstream decode fix is in progress, not finalized.

**Public artifacts.** Full validation (FINDING.md + 3 probe scripts + 4 run logs): https://gist.github.com/Sahil170595/062d40cb18e2b2e27e99c1efbfa3ccdb . Endorsement comment on #184102 (real-decode + topology validation, links the gist): https://github.com/pytorch/pytorch/pull/184102#issuecomment-4635948508 . Related: #154824 (broader umbrella: cudagraph tree may release input nodes during replay) remains open; #175557 is what #184102 Fixes.

**Net correction.** The report's measurements stand: on stock PyTorch (2.8 and 2.10), `reduce-overhead` compiled decode with `DynamicCache` does crash 100%, and every model-layer/never-free path this report tried failed. What changes is the *forward-looking conclusion*: compiled decode is not an architectural dead end. Input-preservation in PyTorch internals (#184102, in review) fixes it, validated on a real gpt2 decode and 7 topologies, with one synthetic multi-partition gap reported upstream.
...
| TR126 v1 | "use `mode=default` for decode" suggestion | **Superseded** — TR126 v2 shows mode="default" also crashes (Inductor uses CUDA graph trees internally). On a stock PyTorch, no torch.compile mode enables decode. (Update 2026-06-05: upstream fix #184102, in review, enables it -- SS10.9.) |
| TR126 v1 | Decode recommendation had "untested" caveat | **Resolved (for stock PyTorch)** — Compiled decode is fully characterized: crashes at 128 tokens on all modes on stock PyTorch; no speedup even at 64 tokens where it works. The Ollama decode recommendation stands for a stock PyTorch. (Update 2026-06-05: the crash is fixable from PyTorch internals via #184102, in review -- SS10.9.) |
...
- [ ] Use `mode="reduce-overhead"` ONLY for prefill; use eager (no compile) for decode — `mode="default"` also crashes on a stock PyTorch
...
> The decode constraint above applies to stock PyTorch as of this writing. The upstream input-preservation fix ([#184102](https://github.com/pytorch/pytorch/pull/184102), in maintainer review) makes compiled `reduce-overhead` decode run to completion; once it merges and ships, the "eager-only for decode" line can be revisited. See SS10.9. (2026-06-05)
...
2. **~~No `mode="default"` for decode~~ (RESOLVED in v2):** v2 tested `mode="default"` — it also crashes due to PyTorch 2.8 Inductor's internal CUDA graph tree management (SS10.5). Additionally, Phase 2 baseline shows compiled decode provides no speedup even when it works (SS5.6). This limitation is resolved for the question as scoped: compiled decode is not viable on a **stock** PyTorch 2.8. (Update 2026-06-05: on a PyTorch carrying input-preservation fix #184102, in review, compiled decode does run -- SS10.9.)
...
- **~~Compiled decode investigation~~ (RESOLVED UPSTREAM 2026-06-05, in review):** v2's `mode="default"` experiment crashes; v3's StaticCache + `mode="default"` works but is 5.8x slower than eager; `reduce-overhead` + StaticCache still crashes; the three-patch "never free" prototype on `cudagraph_trees.py` still crashed. The original conclusion that "manual CUDA graph capture bypassing Inductor is the only remaining viable path" was wrong: PR [pytorch/pytorch#184102](https://github.com/pytorch/pytorch/pull/184102) (jansel, Fixes #175557) fixes compiled `reduce-overhead` decode with a growing `DynamicCache` from inside PyTorch -- validated on a real gpt2 decode and 7 topologies (SS10.9). It is under maintainer review (eellison: likely make it opt-in; needs to interact with cached outputs). One synthetic multi-partition gap remains (reported). (Note: TR127 in the research roadmap is assigned to Long-Context Performance Characterization, not compiled decode.)
- **Upstream fix for cudagraph_trees.py (CORRECTED 2026-06-05):** We traced the crash to `dealloc_current_path_weakrefs()` in `torch/_inductor/cudagraph_trees.py` and filed [pytorch/pytorch#175557](https://github.com/pytorch/pytorch/issues/175557). The assertion fix [pytorch/pytorch#175562](https://github.com/pytorch/pytorch/pull/175562) **landed** in main (commit `be90a1495310`, 2026-06-04). The earlier claim that the crash is "architectural, not patchable in cudagraph_trees.py alone" was wrong -- it described only the "never free" direction (disable `_free_And_Remove_DeleterFn` + `check_memory_pool`), which fails. The decode fix takes the opposite direction: preserve/clone the cudagraph-pool-aliased inputs before dealloc frees them. PR [pytorch/pytorch#184102](https://github.com/pytorch/pytorch/pull/184102) implements this in PyTorch internals (no model/cache-layer change required), validated on real decode; it is under maintainer review. See SS10.9 (and SS10.7 for the superseded prototype results).

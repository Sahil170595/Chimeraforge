    ...
    async with client.stream("POST", f"{base_url}/api/generate", json=payload, timeout=120.0) as response:
        ...
        async for chunk in response.aiter_bytes():
            ...
            try:
                ...
                # For profiling, we assume standard behavior but measure the raw bytes processing
                # In a real scenario, we'd parse line by line.
...
def parse_args():
    ...
    # Inherit args from base but we need to redefine them or use parse_known_args 
    # to add our own. For simplicity, we'll just add ours to a new parser that matches base.
    # Actually, let's just use the base parser and add arguments if possible, 
    # but base_parse_args returns Namespace, not parser.
    # We'll re-implement a simple parser for this specific script.
...
async def main():
    ...
    # Client with optional buffer size (if supported by httpx version or via transport)
    # httpx.AsyncClient doesn't directly take read_buffer_size in init, 
    # but we can pass it to the transport if needed. 
    # However, standard httpx doesn't expose a clean "read_buffer_size" config easily 
    # without using a custom transport or httpcore. 
    # For Exp 1.2, we might need to patch httpcore or just rely on OS buffers?
    # Actually, httpx defaults to 64KB read buffer in recent versions, but maybe 
    # we can't easily change it without a custom Transport.
    # For now, we will proceed with default client and note if we can't change it easily.
    # Wait, the user asked for "httpx read buffer (Default vs 64KB)".
    # Default is often 64KB in httpcore. Maybe they meant "Larger"?
    # We'll leave it default for Exp 1.1.
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        for i in range(1, args.runs + 1):
            ...
            # Dump run metrics
            (output_root / f"run_{i}.json").write_text(json.dumps(run, indent=2))

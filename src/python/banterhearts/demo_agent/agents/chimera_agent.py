"""
Chimera-optimized agent implementation for demo.

Uses top-performing configurations from Technical Report 108
for optimal performance demonstration.
"""
...
try:  # pragma: no cover
    ...
    from ..config.chimera_config import (
        get_chimera_config_for_model,
    )
    from ..metrics.collector import MetricsCollector
except ImportError:  # pragma: no cover
    from agents.base_agent import BaseAgent, BenchmarkData, AnalysisResult  # type: ignore
    from config.chimera_config import (  # type: ignore
        get_chimera_config_for_model,
    )
...
class ChimeraAgent(BaseAgent):
    """
    Chimera-optimized agent using top-performing configurations.

    Uses optimal settings from Technical Report 108 for maximum performance.
    Implements identical workflow as BaselineAgent for fair comparison.
    """
    ...
    def __init__(
        ...
        chimera_config = get_chimera_config_for_model(model, overrides=overrides)
        super().__init__(model, chimera_config.to_ollama_options(), base_url=base_url)

        self.chimera_config = chimera_config
    ...
    async def analyze_data(self, benchmark_data: List[BenchmarkData]) -> AnalysisResult:
        """Process and analyze benchmark data using Chimera-optimized LLM."""
        self.metrics_collector.start_collection()

        # Create analysis prompt with Chimera context
        data_summary = self._create_data_summary(benchmark_data)

        analysis_prompt = f"""
You are a performance analysis expert with access to Chimera optimization data.
Analyze the following benchmark data and provide insights, referencing the
comprehensive benchmarking results from Technical Report 108.

Benchmark Data Summary:
{data_summary}

Chimera Optimization Context:
- This analysis uses Chimera-optimized configuration: {self.chimera_config.get_description()}
- Expected performance: {self.chimera_config.expected_throughput_tok_s} tok/s throughput
- Configuration rationale: {self.chimera_config.get_citations()}
...
"""
        ...
        # Get LLM analysis with Chimera optimization
    ...
    async def _call_llm(self, prompt: str) -> str:
        """
        Make a Chimera-optimized LLM call with metrics collection.

        Sends a prompt to the Ollama API using the agent's Chimera configuration
    ...
    def _extract_findings(self, response: str) -> List[str]:
        ...
        "key", or "chimera" and returns up to 10 findings.
        ...
        for line in lines:
            if any(
                ...
                for keyword in ["finding", "insight", "discovery", "key", "chimera"]
            ):
    ...
    def _extract_metrics(self, benchmark_data: List[BenchmarkData]) -> Dict[str, Any]:
        ...
        performance metrics from CSV data. Includes Chimera optimization context.
        ...
        Returns:
            Dict[str, Any]: Dictionary containing:
                ...
                - chimera_optimization: Chimera config and expected metrics
        ...
        metrics = {
            ...
            "chimera_optimization": {
                "expected_throughput": self.chimera_config.expected_throughput_tok_s,
                "expected_ttft": self.chimera_config.expected_ttft_s,
    ...
    def _extract_recommendations(self, response: str) -> List[str]:
        ...
        "suggest", "should", "consider", or "optimize" and returns up to 10.
    ...
    async def generate_report(self, analysis: AnalysisResult) -> str:
        """Generate Technical Report 108-style markdown report with Chimera insights."""
        report_prompt = f"""
Generate a comprehensive technical report in the style of Technical Report 108,
focusing on Chimera optimization benefits and performance analysis.
...
{self.chimera_config.get_description()}

Citations:
{self.chimera_config.get_citations()}

Create a professional technical report with the following structure:
1. Executive Summary (highlighting Chimera optimization benefits)
...
Use markdown formatting and include specific metrics, performance comparisons,
and references to Technical Report 108 findings.
"""

        report_content = await self._call_llm(report_prompt)

        # Add header information with Chimera context
        header = f"""# Technical Report: Chimera-Optimized Agent Analysis

**Date:** {datetime.now().strftime("%Y-%m-%d")}
**Agent Type:** Chimera-Optimized (Technical Report 108 Configuration)
**Model:** {self.model}
**Configuration:** {self.chimera_config.get_description()}
...
- Throughput: {self.chimera_config.expected_throughput_tok_s} tok/s
- TTFT: {self.chimera_config.expected_ttft_s}s
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "chimera_config": {
                "expected_throughput": self.chimera_config.expected_throughput_tok_s,
                "expected_ttft": self.chimera_config.expected_ttft_s,
                "description": self.chimera_config.get_description(),
                "citations": self.chimera_config.get_citations(),
            },
